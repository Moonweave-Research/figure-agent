...
# sty 디렉토리를 TEXINPUTS에 자동 포함 (polymer-paper-preamble.sty 찾기용)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WORKFLOW_DIR="$(dirname "$SCRIPT_DIR")"
REPO_ROOT="$WORKFLOW_DIR"
export TEXINPUTS="${WORKFLOW_DIR}/sty/:${TEXINPUTS:-}"
...
if [[ $# -lt 1 ]]; then
  echo "Usage: $(basename "$0") [--fig-name <name>] [--no-ref=\"<reason>\"] <file.tex>" >&2
  exit 1
fi
...
if [[ -z "$FIG_NAME" ]]; then
  FIG_NAME="$(python3 - "$REPO_ROOT" "$TEX_INPUT" <<'PY'
import pathlib
import sys

repo_root = pathlib.Path(sys.argv[1]).resolve()
tex_path = pathlib.Path(sys.argv[2]).resolve()
references = repo_root / "references"
try:
    rel = tex_path.relative_to(references)
except ValueError:
    print("")
else:
