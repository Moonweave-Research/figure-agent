...
def allowlist_binding_failures(
    ...
    word and the checker used to print OK.  Resolving the list as a whole hid
    the same defect one word at a time — a word the render never draws stopped
    selecting anything while its neighbours kept the list alive — so each
    declared word has to bind on its own.
    """
    failures: list[dict[str, Any]] = []
    for check in sorted(checks, key=_check_sort_key):
        allowlist = _text_allowlist(check)
