...
GENERATION_RECEIPT_SCHEMA = "figure-agent.generation-receipt.v2"
REPAIR_LINEAGE_SCHEMA = "figure-agent.bounded-repair-lineage.v1"
REPAIR_PACKET_SCHEMA = "figure-agent.repair-execution-packet.v3"
HUMAN_DECISION_SCHEMA = "figure-agent.human-decision-record.v1"
...
def _has_bound_generation_receipt(run: dict[str, Any]) -> bool:
    ...
    if not (
        ...
        and receipt.get("shared_task_sha256") == run.get("shared_task_hash")
    ...
    if actual_hash != declared_hash:
        return False
    input_packet_value = receipt.get("input_packet_path")
    if not isinstance(input_packet_value, str):
        return False
    input_packet_relative = Path(input_packet_value)
    if input_packet_relative.is_absolute() or len(input_packet_relative.parts) != 1:
        return False
    input_packet = run_path.parent / input_packet_relative
    if input_packet.is_symlink() or not input_packet.is_file():
        return False
    if (
        f"sha256:{hashlib.sha256(input_packet.read_bytes()).hexdigest()}"
        != receipt.get("input_packet_sha256")
    ):
        return False
    try:
        ...
        return False
    if not (
        packet.get("model_id") == receipt.get("model_id")
        and shared_task_sha256 == receipt.get("shared_task_sha256")
        and context_pack_base_sha256 == receipt.get("context_pack_base_sha256")
    ):
        return False
...
def _bound_adjacent_artifact(
    ...
    *,
    ...
    if len(relative_path.parts) == 1:
        artifact = run_path.parent / relative_path
    elif workspace_root is not None:
        artifact = workspace_root / relative_path
        try:
            artifact.resolve(strict=False).relative_to(workspace_root.resolve())
        except ValueError:
            return None
    else:
        return None
...
def _json_mapping(artifact: tuple[Path, bytes] | None) -> dict[str, Any] | None:
    if artifact is None:
        return None
    try:
...
def _repair_attribution_is_bound(
    *,
    ...
    if (
        ...
        != "figure-agent.repair-target-contract.v1"
...
def _has_bound_repair_lineage(
    *,
    ...
    """Return whether repaired is a finalized, human-authorized child of verified."""
    ...
    if not (
        repair_packet.get("schema") == REPAIR_PACKET_SCHEMA
        and packet_hash
        ...
        and repair_packet_artifact is not None
...
def evaluate_ablation(
    ...
    shared_task_states = {
        name: runs[name].get("shared_task_hash") for name in VARIANTS
    }
    ...
    if any(not runs[name].get("input_packet_hash") for name in VARIANTS):
        raise FailureAblationError("comparison_contract_mismatch")
    keys = (
        "model_contract_hash",
    ...
    repair_lineage_bound = authoring_transcript_bound and _has_bound_repair_lineage(
    ...
    return {
        ...
        "comparison_evidence": (
            ...
            if repair_lineage_bound
            else (
                "authoring_transcript_bound_only"
                if authoring_transcript_bound
                else "staged_only"
            )
        ),
        "repair_lineage_evidence": (
            "bound" if repair_lineage_bound else "missing_or_invalid"
        ),
