...
QUALITY_SEARCH_FAMILY_REGISTRY = {
    ...
    "panel_f_trap_label_left_rail": {
        ...
        "design_moves": [
            "move the trap-label pair onto a stable left-margin rail",
            "shorten and lighten the trap leader so it does not dominate the cantilever",
            "preserve force, electrode, air-gap, and source semantics",
        ],
        "render_targets": ["full", "print_thumbnail", "panel_F"],
    },
...
def _goal_hypotheses(name: str, goal: str) -> list[dict[str, Any]]:
    ...
    if any(
        term in normalized
        for term in ("force", "coulomb", "repulsion", "arrow", "air gap", "electrode")
    ) and not any(term in normalized for term in ("apparatus", "strengthen")):
        hypotheses.insert(
            0,
            {
                "fixture": name,
                "family": "panel_f_post_boundary_force_balance",
                "source": "goal_directive",
                "mutation_allowed": False,
                "mutation_block_reason": "quality-search v0 is planner-only",
                "target_scope": "panel",
                "target_hint": {
                    "panels": ["F"],
                    "reason": (
                        "goal explicitly requests post-boundary Panel F force, "
                        "arrow, Coulomb/repulsion, or air-gap balance"
                    ),
                },
                "expected_detector_movement": (
                    "convert post-boundary force-label and arrow-spacing defects "
    ...
    if any(term in normalized for term in ("after source", "post source", "final")) and any(
        term in normalized
        for term in ("trapped charge", "q_tr", "label", "background", "scale", "polish")
    ):
        hypotheses.insert(
            0,
            {
                "fixture": name,
                "family": "panel_f_post_source_label_scale",
                "source": "goal_directive",
                "mutation_allowed": False,
                "mutation_block_reason": "quality-search v0 is planner-only",
                "target_scope": "panel",
                "target_hint": {
                    "panels": ["F"],
                    "reason": (
                        "goal explicitly requests post-source Panel F trapped-charge "
                        "label scale and background polish"
                    ),
                },
                "expected_detector_movement": (
                    "convert latest trapped-charge label scale/background defects "
                    "into a fresh source-bound Panel F candidate"
                ),
                "expected_visual_movement": (
                    "q_tr and trapped charge become slightly more legible while "
                    "remaining in the left label rail"
                ),
                "rollback_condition": (
                    "candidate worsens compile, protected labels, source attachment, "
    ...
    if any(
        term in normalized
        for term in (
            "after gap readability",
            "post gap readability",
            "after electrode",
        )
    ) and any(
        term in normalized
        ...
        hypotheses.insert(
            ...
            {
                "fixture": name,
                "family": "panel_f_post_gap_label_relief",
                "source": "goal_directive",
                "mutation_allowed": False,
                "mutation_block_reason": "quality-search v0 is planner-only",
                "target_scope": "panel",
                "target_hint": {
                    "panels": ["F"],
                    "reason": (
                        "goal requests Panel F label-background and source-lead "
                        "relief after electrode/air-gap readability cleanup"
                    ),
                },
                "expected_detector_movement": (
                    "convert post-gap-readability label/background/source-lead "
                    "defects into a fresh source-bound Panel F candidate"
                ),
                "expected_visual_movement": (
                    "trapped-charge label background stops reading as an underline "
                    "while source lead and force rail become quieter"
                ),
                "rollback_condition": (
                    "candidate worsens compile, protected labels, force direction, "
    ...
    if any(
        ...
        hypotheses.insert(
            ...
            {
                "fixture": name,
                "family": "panel_f_trap_label_left_rail",
                "source": "goal_directive",
                "mutation_allowed": False,
                "mutation_block_reason": "quality-search v0 is planner-only",
                "target_scope": "panel",
                "target_hint": {
                    "panels": ["F"],
                    "reason": (
                        "goal requests Panel F trap labels moved to a left rail "
                        "with a quieter leader away from the cantilever"
                    ),
                },
                "expected_detector_movement": (
                    "convert post-source-settle trap label routing into a fresh "
                    "left-margin rail candidate"
                ),
                "expected_visual_movement": (
                    "q_tr and trapped charge form a clearer left label rail while "
                    "the leader reads as secondary context"
                ),
                "rollback_condition": (
                    "reject if protected labels disappear or the trap leader "
                    "crosses the cantilever body"
                ),
            },
        )
...
def _panel_f_post_source_label_scale_replacement(
    *,
    lines: list[str],
    selector: dict[str, Any],
) -> tuple[str, str, int, int] | None:
    ...
    if not _panel_f_overlay_has_protected_labels(original):
        return None
    if not _panel_f_post_force_source_connector_template_applied(original):
        return None
    if _panel_f_post_source_label_scale_template_applied(original):
        return None
    replacements = (
        (
            "% quality-search F boundary polish: pull callouts off panel edge\n"
    ...
    if not all(label in replacement for label in protected):
        return None
    if not _panel_f_post_source_label_scale_template_applied(replacement):
        return None
    return original, replacement, line_start, line_end
...
def _panel_f_post_gap_label_relief_template_applied(block: str) -> bool:
    required_fragments = (
...
def _panel_f_post_gap_label_relief_replacement(
    *,
    lines: list[str],
    selector: dict[str, Any],
) -> tuple[str, str, int, int] | None:
    ...
    if not _panel_f_overlay_has_protected_labels(original):
        return None
    if not _panel_f_post_trap_gap_readability_template_applied(original):
        return None
    if _panel_f_post_gap_label_relief_template_applied(original):
        return None
    replacements = (
        (
            "% quality-search F post-spacing source finish: soften source lead\n"
        ...
        (
            ...
            "\\node[anchor=west, fill=white, fill opacity=0.90, text opacity=1,\n"
            "      inner xsep=0.82pt, inner ysep=0.34pt,\n"
            "      font=\\sffamily\\bfseries\\fontsize{3.9}{4.7}\\selectfont, text=cRed!72!black]\n"
            "  at (9.64, 3.56) {trapped charge};",
            "\\draw[cRed!52!black, line width=0.28pt]\n"
        ...
        (
            "\\draw[panelFCoulombRepulsionArrow, "
            "-{Stealth[length=6.6pt,width=4.5pt]}, "
            "cRed!74!black, line width=0.68pt]\n"
            "  (10.58, 1.08) -- (9.66, 1.08);",
            "\\draw[panelFCoulombRepulsionArrow, "
            "-{Stealth[length=6.2pt,width=4.2pt]}, "
        ...
        (
            "font=\\sffamily\\bfseries\\fontsize{4.9}{5.9}\\selectfont, text=cRed!74!black]\n"
            "  at (9.58, 1.70) {Coulomb};",
            "font=\\sffamily\\bfseries\\fontsize{4.8}{5.8}\\selectfont, text=cRed!72!black]\n"
    ...
    if not all(label in replacement for label in protected):
        return None
    if not _panel_f_post_gap_label_relief_template_applied(replacement):
        return None
    if not _panel_f_post_trap_gap_readability_template_applied(replacement):
        return None
    return original, replacement, line_start, line_end
...
def _candidate_operation_for_spec(
    ...
    *,
    ...
    if family == "panel_f_post_source_label_scale":
        label_scale_block = _panel_f_post_source_label_scale_replacement(
            lines=lines,
            selector=selector,
        )
        if label_scale_block is not None:
            original, new_text, line_start, line_end = label_scale_block
            operation = {
                "kind": "replace_text",
                "semantic_kind": "quality_search_panel_f_post_source_label_scale_panel_block",
                "operation_scale": "panel_block",
            ...
            return operation, None
        return None, {
            "code": "no_panel_f_post_source_label_scale_block",
            "candidate_id": str(spec.get("id")),
            "family": family,
            "operation_scale": "panel_block",
            "template_id": PANEL_F_POST_SOURCE_LABEL_SCALE_TEMPLATE_ID,
    ...
    if family == "panel_f_post_gap_label_relief":
        label_relief_block = _panel_f_post_gap_label_relief_replacement(
            lines=lines,
            selector=selector,
        )
        if label_relief_block is not None:
            original, new_text, line_start, line_end = label_relief_block
            operation = {
                "kind": "replace_text",
                "semantic_kind": "quality_search_panel_f_post_gap_label_relief_panel_block",
                "operation_scale": "panel_block",
            ...
            return operation, None
        return None, {
            "code": "no_panel_f_post_gap_label_relief_block",
            "candidate_id": str(spec.get("id")),
            "family": family,
            "operation_scale": "panel_block",
            "template_id": PANEL_F_POST_GAP_LABEL_RELIEF_TEMPLATE_ID,
