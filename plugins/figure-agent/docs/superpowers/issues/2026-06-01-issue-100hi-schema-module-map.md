...
| `figure-agent.fixture-driver-queue.v1` / `figure-agent.fixture-command-plan.v1` / `figure-agent.queue-bottleneck-report.v1` | batch planning | `fig_queue.py` | operators, `fig_queue_run.py` | Must preserve per-fixture gates; bottleneck report is read-only live queue/status rollup |
