...
| `figure-agent.attempt-local-post-repair-review-request.v2` | R4.14 attempt-local post-repair review request | `attempt_local_post_review.py` | external human/host review workflow and `attempt_local_post_review_response.py` | Revalidates the current v2 machine-repaired packet, binding, receipt, materialized output, before/after render, exact initial six-crop provenance, and deterministic after-render crop pack. Plan-only is write-free; execute atomically publishes only the request/crops and `post_review_requested` state, then stops before host/model invocation without inventing a target bbox or claiming publication acceptance. |
| `figure-agent.attempt-local-host-review-execution-receipt.v2` | R4.14 exact host-review execution evidence | `attempt_local_post_review_response.py` | attempt-local response pack validation | Binds the named host/model execution to the exact request hash and inspected artifact pack. It is response-pack evidence, not lifecycle authorization; missing execution keeps the attempt write-free at `post_review_requested` for human completion or clarification. |
| `figure-agent.attempt-local-post-repair-review-response.v2` | R4.14 attempt-local post-repair review response | `attempt_local_post_review_response.py` | `fig_run.py` v2 inbound dispatch | Binds the exact request, host transcript, execution receipt, and three required verdicts. Uncertain or incomplete evidence remains write-free at the human-review boundary; explicit defects route to `repair_required`; only a complete pass can publish `visually_re_reviewed`. |
| `figure-agent.attempt-local-post-repair-review-receipt.v2` | R4.14 validated response receipt | `attempt_local_post_review_response.py` | attempt-local state publication and receipt-only recovery | Immutable receipt for a fully validated v2 response pack. It cannot claim publication acceptance or substitute for the later named human development verdict. |
...
| `figure-agent.claim-authority.v1` | fixture-local epistemic claim boundary | `claim_authority.py` | `authoring_context_pack.py`, `status.py`, `status_explanation.py`, `next_action_summary.py`, `compile.sh` render-input binding | Supported/schematic/hypothesis claims remain qualified inputs; unresolved, conflicted, or forbidden claims stop authoring for explicit human resolution and never become inferred scientific instructions |
| `figure-agent.semantic-contract-evidence.v1` | hash-bound validated semantic contract receipt | `quality/semantic_legibility_contract.py` | `status.py`, `compile.sh` | Build evidence only; never implies semantic render coverage or publication acceptance |
...
| `figure-agent.failure-first-semantic-contract.v1` | standalone failure-first semantic context | `authoring_context_pack.py` | `authoring_context_pack.py`, authoring packet tests | Fixture-local semantic claims/invariants are adapted into the authoring context pack; no model call or publication authority |
| `figure-agent.generation-receipt.v2` | packet-bound generation evidence | `quality/generation_receipt.py` | `quality/failure_ablation.py`, ablation operators | Binds a generation transcript to an adjacent authoring packet, neutral-task hash, and shared context base; evidence only, never acceptance |
| `figure-agent.bounded-repair-lineage.v1` | packet-bound repair-child evidence | `quality/failure_ablation.py` and repair materialization workflow | `quality/failure_ablation.py`, ablation operators | Requires one exact parent finding/selector, hash-bound packet/output/receipt, external strict verification, and named additive-materialization authorization; evidence only, never acceptance |
...
| `figure-agent.closed-loop-terminal-experience.v1` | closed-loop terminal experience receipt | `experience_log.py` | `quality_memory_index.py`, convergence/learning reports | Evidence-bound terminal attempt outcome; records observed pipeline, detector, human, and regression signals without asserting cross-family promotion |
...
| `figure-agent.compile-run.v1` | receipt for one compile run | `compile_run.py`, `compile.sh` | `render_input_manifest.py`, `status.py` | Names the run id, source, and render hash of a single compile; every other build receipt must carry that run id, and a receipt whose render no longer sits in `build/` binds nothing |
| `figure-agent.render-input-manifest.v2` | hash-bound compile inputs | `render_input_manifest.py`, `compile.sh` | `status.py` | Binds PDF/PNG bytes to the compile receipt, TeX recorder dependencies, toolchain, briefing, spec, and Style Lock content; identical plugin-cache copies stay fresh while byte drift fails closed |
...
| `figure-agent.submission-bundle.v1` | editable delivery | `submission_bundle.py` | export consumers | PDF submission role, editable TeX/dependencies, hashes and build receipts |
