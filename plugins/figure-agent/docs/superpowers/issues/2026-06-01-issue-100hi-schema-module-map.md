...
| `figure-agent.three-way-blind-review.v2` | direct-SVG blinded review | `direct_svg_stage_review.py` | named human reviewers, `direct_svg_stage_review.py` replay/export | Public score inputs use seed-bound opaque commitments; diagnostics and machine gates cannot claim publication acceptance |
...
| `figure-agent.direct-svg-review-state.v3` | direct-SVG protected review state | `direct_svg_stage_review.py` | `direct_svg_stage_review.py` replay/advance/reveal | Stores fixed response hashes and blocks unblinding until the finalized response validates |
| `figure-agent.private-three-way-review-key.v2` | direct-SVG protected blinding key | `direct_svg_stage_review.py` | locked replay and post-finalization reveal only | External ignored 0700/0600 evidence; raw upstream hashes, seed, and assignments never enter the public distribution before reveal |
