...
| `figure-agent.human-decision-packet.v1` / `figure-agent.release-decision-packet.v1` / `figure-agent.style-direction-packet.v1` / `figure-agent.svg-polish-readiness-evidence.v1` / `figure-agent.human-decision-digest.v1` | human/release/style decision handoff | `fig_queue.py` | operators | Read-only recommendation and digest packets; source, SVG, accepted/golden/publication mutations remain explicit separate actions |
