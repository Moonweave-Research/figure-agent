...
| `figure-agent.attempt-local-repair-binding.v2` / `figure-agent.attempt-local-repair-packet.v1` | R4.13a same-attempt repair authority | `attempt_local_repair_binding.py`, `authoring_repair_packet.py` | same modules | Revalidates initial request/response, adjudication handoff, v2 binding, root source/render, and exact six crops; creates only immutable binding/packet/prompt/source sandbox, never a model call, response, materialization, state advance, or publication claim |
