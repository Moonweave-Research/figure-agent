...
| `figure-agent.fixture-driver-queue.v1` / `figure-agent.fixture-command-plan.v1` / `figure-agent.queue-bottleneck-report.v1` / `figure-agent.queue-workspace-diagnostic.v1` | batch planning | `fig_queue.py` | operators, `fig_queue_run.py` | Must preserve per-fixture gates; bottleneck report is read-only live queue/status rollup; workspace diagnostic only explains missing examples discovery |
...
| `figure-agent.human-decision-packet.v1` / `figure-agent.release-decision-packet.v1` | human/release decision handoff | `fig_queue.py` | operators | Read-only recommendation packet; accepted/golden/publication mutations remain explicit human-only actions |
