...
svg_polish_gate:
  ...
  source: fig_loop_checkpoint | status_blocker | driver_blocker
  reason: "<one-line>"
  required_inputs:
    - critique_fresh
...
must remove operator ambiguity.
...
manifest but omits this block, does not account for all required delta images,
or reports a regression without a visible finding / human route.
...
17. Host critique does not read all delta images.
    - Expected: lint rejects `read_all_delta_images: false` or missing image ids.
...
21. The SVG polish adds decorative texture that makes the figure look more
    "handmade" but violates journal restraint.
    - Expected: route to human art direction or regression, not automatic pass.
22. The SVG polish improves label spacing but shifts a scientific arrow.
    - Expected: semantic drift blocks acceptance.
23. The SVG polish changes color/opacity in a way that breaks palette
    semantics.
    - Expected: semantic or journal-fit regression.
24. The SVG polish makes the full figure prettier but worsens print-scale
    readability.
    - Expected: print-scale regression blocks.
25. The SVG polish improves one panel but hurts paper-wide coherence.
    - Expected: require paper-wide coherence accounting when context exists.
26. The aesthetic issue is truly subjective.
    - Expected: `needs_human_art_direction`, not forced patching.
27. The figure is already release/golden-ready without SVG polish.
    - Expected: do not require SVG polish; preserve release route.
28. The host says "more premium" or "more beautiful" without naming the closed
    aesthetic slot and current artifact evidence.
    - Expected: invalid generic prose.
29. The host says "handcrafted_finish fail" but the journal playbook asks for
...
30. Legacy critiques without SVG delta remain parseable.
    - Expected: no new required block unless a fresh delta manifest exists or
      schema vNext opts in.
31. Existing v1.14 critiques remain valid unless the fixture has fresh delta
    inputs that require the new contract.
32. Fixtures with no `aesthetic_intent.yaml`, no paper context, and no journal
    playbook still work.
    - Expected: use generic closed-set gate, but do not require missing packs.
33. Fixtures with `aesthetic_intent.yaml` v1 continue using legacy behavior.
34. Existing SVG polish recipe tests continue to pass.
35. Existing queue/run/export/release gates remain unchanged.
36. Accepted tracked-golden fixtures do not become release-blocked merely
    because they lack SVG polish evidence.

### Operator Safety Edge Cases

37. `/fig_driver --mode polish` must not expose a write command when readiness
    is blocked.
38. `/fig_queue` must classify SVG polish rows as `svg_editor` or a blocked
    operator handoff, not workflow-agent execution.
39. `/fig_run` must not execute SVG polish writes unless a later issue
    explicitly expands its allowlist.
40. `safe_command` strings remain evidence, not permission to replay stale
    state.
41. Accepted/golden state must never be mutated by SVG polish gate checks.
42. Plan-only queue output may list SVG handoff commands, but those commands
...
- No schema bump required if additive.
...
- Tests: missing block, stale delta, missing image id, regression without
  linked finding, legacy compatibility.
