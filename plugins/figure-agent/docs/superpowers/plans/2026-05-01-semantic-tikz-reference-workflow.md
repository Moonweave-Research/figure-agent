...
- Sub-task B deviations applied at commit time (post-plan-finalization): (1) §6 reference disambiguated to `§6 Physics invariants (Briefing Context)` — addresses ambiguity from two `## 6.` headings in the prompt. (2) Step 1 verb deduplication: removed the second `Internally decompose` clause; epilogue `Do not include the decomposition in the output; emit only the .tex body per §1` now stands alone. (3) §2 third bullet collapsed from a 3-line block back to a single bullet so the `Do not emit raw traced path clouds` substring lives contiguously, satisfying the strengthened forbids assertion without disrupting §2's other-bullet wrap convention. The plan's Task 5 Step 2 markdown body shown above predates these fixes; the applied prompt is the source of truth.
- Coordinate-hints role assertion isolated: The `Use coordinate_hints.yaml as placement evidence` substring (originally locked in `test_authoring_prompt_forbids_path_dump_final_source`) was migrated to a dedicated `test_authoring_prompt_documents_coordinate_hints_role` function during integrated review. Reason: after the §5b body replacement, that phrase survives only in §7 Output Contract, so coupling it to the forbids axis would silently re-anchor the forbids test to §7 changes. Single-axis tests stay diagnostic.
