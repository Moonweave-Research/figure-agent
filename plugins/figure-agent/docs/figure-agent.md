...
An approved initial review requires one human-authored, attempt-local v2 attribution binding that hash-binds the selected finding, handoff, root authored source, selector registry, and semantic contract; R4.13a derives a same-attempt immutable authority plus packet/prompt/source sandbox at `repair_bound`, but invokes no model, response, materialization, render, host, or state advance.
