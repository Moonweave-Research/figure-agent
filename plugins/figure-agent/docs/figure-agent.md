...
R4.14 now publishes a separate attempt-local v2 post-repair request from that machine-verified state. It preserves the initial inspection pack: legacy attempts retain the original six views, while new attempts add vertical- and horizontal-seam views so a semantic unit centered on a quadrant boundary remains inspectable as a whole. It creates deterministic after-render crops, advances only to `post_review_requested`, and stops at the external review boundary; it does not reuse the legacy target/neighbor-bbox request contract or infer a visual verdict.
