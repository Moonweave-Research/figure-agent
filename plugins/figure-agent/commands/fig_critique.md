...
4. Use the **Write** tool to create `examples/<name>/critique.md` with the exact YAML front-matter format printed by the brief. Fixtures without `aesthetic_intent.yaml` or with schema v1 use the v1.10 shape below. Fixtures with `figure-agent.aesthetic-intent.v2` use the v1.11 shape printed by the brief and include `aesthetic_lever_audit`; fixtures with `spec.yaml.journal_art_direction_playbook` use the v1.12 shape and include `journal_art_direction_playbook_audit` plus `aesthetic_lever_audit` when v2 aesthetic intent is also present. Do not paste older schemas for opted-in fixtures.
