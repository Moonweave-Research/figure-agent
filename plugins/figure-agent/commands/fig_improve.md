---
description: Compatibility loop wrapper over /fig_run. Runs safe mechanical steps and stops at review, repair, and human boundaries.
---
...
`/fig_improve` is a compatibility wrapper for repeated single-fixture requests.
The canonical executor is `/fig_run`; improve calls that bounded workflow and
summarizes where the loop stopped.
...
boundaries. After the required actor acts, rerun
`/fig_improve`.
