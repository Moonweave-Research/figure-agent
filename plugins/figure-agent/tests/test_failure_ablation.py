...
NEUTRAL_TASK = "# Neutral task\n\nDraw the declared scientific figure.\n"
        ...
        yaml.safe_dump(
            {
                ...
                "shared_task_hash": "sha256:"
                + hashlib.sha256(NEUTRAL_TASK.encode()).hexdigest(),
...
def add_generation_receipt(path: Path, *, model_id: str = "test-model") -> None:
    ...
    input_packet_path = path.with_name(f"{path.stem}.packet.json")
    prompt = (
        "# Authoring execution\n\n"
    ...
    budget_contract_path = path.with_name("budget.yaml")
    budget_contract_path.write_text("attempts: 1\n", encoding="utf-8")
    ...
    payload["input_packet_hash"] = _sha256(input_packet_path)
    payload["budget_contract_hash"] = _sha256(budget_contract_path)
    ...
    record_generation_receipt(
        ...
        budget_contract=budget_contract_path,
        starting_artifact=starting_artifact_path,
        generated_artifact=generated_artifact_path,
    )
...
def add_repair_lineage(
    ...
    *,
    ...
    packet_payload: dict[str, object] = {
        ...
        "target_contract": {"path": target_contract.name, "sha256": _sha256(target_contract)},
        "finding_reports": [
            {
    ...
    repair_packet.write_text(json.dumps(packet_payload, sort_keys=True), encoding="utf-8")
...
def _bound_repair_paths(tmp_path: Path, **lineage_options: object) -> dict[str, Path]:
    ...
    add_generation_receipt(paths["raw"])
    add_generation_receipt(paths["verified"])
    add_repair_lineage(paths, **lineage_options)
    return paths


def test_repair_lineage_rejects_independent_repaired_generation_receipt(
    tmp_path: Path,
) -> None:
    paths = write_comparable_runs(tmp_path)
    for path in paths.values():
        add_generation_receipt(path)
    repaired_generation = yaml.safe_load(paths["repaired"].read_text(encoding="utf-8"))[
        "generation_receipt"
    ]
    add_repair_lineage(paths)
    repaired = yaml.safe_load(paths["repaired"].read_text(encoding="utf-8"))
    repaired["generation_receipt"] = repaired_generation
    paths["repaired"].write_text(yaml.safe_dump(repaired), encoding="utf-8")
...
def test_repair_lineage_rejects_noncanonical_minimal_packet(tmp_path: Path) -> None:
    paths = write_comparable_runs(tmp_path)
    add_generation_receipt(paths["raw"])
    add_generation_receipt(paths["verified"])
    add_repair_lineage(paths)
...
def test_repair_lineage_rejects_missing_target_contract(tmp_path: Path) -> None:
...
def test_repair_lineage_rejects_target_contract_parent_drift(tmp_path: Path) -> None:
    paths = _bound_repair_paths(
        tmp_path, contract_source_hash="sha256:" + "8" * 64
    )
...
def test_repair_lineage_rejects_nonunique_exact_selector(tmp_path: Path) -> None:
    paths = _bound_repair_paths(tmp_path, selector_start="% duplicate-marker")
...
def test_repair_lineage_rejects_duplicate_finding_id(tmp_path: Path) -> None:
    paths = _bound_repair_paths(tmp_path, duplicate_finding=True)
...
def test_repair_lineage_rejects_editable_target_contract_drift(tmp_path: Path) -> None:
    paths = _bound_repair_paths(
        tmp_path, editable_repair_family="local_reposition"
    )
...
def test_ablation_allows_distinct_intervention_packets_for_equal_shared_task(
    tmp_path: Path,
) -> None:
    paths = write_comparable_runs(tmp_path)
    for index, path in enumerate(paths.values()):
        payload = yaml.safe_load(path.read_text(encoding="utf-8"))
        payload["shared_task_hash"] = "sha256:" + "4" * 64
        payload["input_packet_hash"] = "sha256:" + str(index + 5) * 64
        path.write_text(yaml.safe_dump(payload), encoding="utf-8")
...
def test_third_independent_generation_is_not_bounded_repair_evidence(
    tmp_path: Path,
) -> None:
    paths = write_comparable_runs(tmp_path)
    for path in paths.values():
...
def test_ablation_accepts_verified_to_repaired_hash_bound_lineage(
    tmp_path: Path,
) -> None:
    paths = write_comparable_runs(tmp_path)
    for path in paths.values():
...
def test_ablation_rejects_repair_lineage_when_parent_hash_drifts(
    tmp_path: Path,
) -> None:
    paths = write_comparable_runs(tmp_path)
    add_generation_receipt(paths["raw"])
    ...
    add_repair_lineage(paths)
    repaired = yaml.safe_load(paths["repaired"].read_text(encoding="utf-8"))
    repaired["repair_lineage"]["parent_generated_artifact_sha256"] = (
        "sha256:" + "9" * 64
    )
    paths["repaired"].write_text(
        yaml.safe_dump(repaired, sort_keys=False), encoding="utf-8"
