...
def test_status_surfaces_zero_declared_candidate_coverage_as_a_gap(tmp_path: Path) -> None:
    ...
    fig_dir.mkdir()
    _make_spec(fig_dir)
    (fig_dir / f"{fig_dir.name}.tex").write_text("root", encoding="utf-8")
    (fig_dir / "build").mkdir()
    source = fig_dir / "review" / "candidate-c1" / "repaired.tex"
    source.parent.mkdir(parents=True)
    source.write_text("candidate", encoding="utf-8")
    ...
    build.mkdir()
    for name, payload in {
        "repaired.pdf": b"candidate-pdf",
        "repaired.png": b"candidate-png",
        "strict_status.json": {
        ...
        "text_boundary_clash.json": {"checked": 0, "candidates": []},
        "label_path_proximity.json": {"checked": 0, "candidates": []},
    }.items():
        path = build / name
    ...
    assert ("current_candidate_text_boundary_coverage", "gap") in result["checks"]
    assert ("current_candidate_label_path_coverage", "gap") in result["checks"]
    assert "current_candidate_text_boundary_coverage_gap" in result["notes"]
    assert "current_candidate_label_path_coverage_gap" in result["notes"]
...
def _write_final_artifact_spec(fig_dir: Path, *, kind: str = "generated_export") -> None:
    ...
    spec.write_text(
        ...
        + "final_artifact:\n"
        + f"  kind: {kind}\n"
        + "  manifest: polish/external-handoff.yaml\n",
        encoding="utf-8",
    )
...
def test_unknown_final_artifact_kind_blocks_release(
    ...
    fig_dir = tmp_path / "external_kind_status"
    _make_status_ready_fixture(fig_dir, accepted=True)
...
def _allowlist_coverage_spine(tmp_path: Path, allowlist_binding: dict | None) -> dict:
    spec = tmp_path / "spec.yaml"
    spec.write_text(
        "text_boundary_checks:\n  - id: title-boundary\n    text_allowlist: [probe]\n",
        encoding="utf-8",
    )
    build = tmp_path / "build"
    build.mkdir(exist_ok=True)
    ...
    if allowlist_binding is not None:
        payload["allowlist_binding"] = allowlist_binding
    (build / "text_boundary_clash.json").write_text(json.dumps(payload), encoding="utf-8")
    return status_mod._spine_evidence_summary(tmp_path, build)


def test_spine_blocks_on_a_declared_allowlist_that_binds_nothing(tmp_path: Path) -> None:
    spine = _allowlist_coverage_spine(
        tmp_path,
        {
            "checked": 1,
            "state": "failed",
            "failures": [
                {
                    ...
                    "detail": "probe (nearest rendered: ESVM, head)",
                }
            ],
        },
    )
    ...
    coverage = spine["text_boundary_coverage"]
    assert coverage["state"] == "needs_action"
    assert coverage["allowlist_binding_state"] == "failed"
    assert coverage["allowlist_binding_checked"] == 1
    assert spine["state"] == "needs_action"


def test_spine_rejects_a_report_that_never_examined_the_declared_allowlists(
    tmp_path: Path,
) -> None:
    assert _allowlist_coverage_spine(tmp_path, None)["text_boundary_coverage"]["state"] == "invalid"
    assert (
        _allowlist_coverage_spine(
            tmp_path, {"checked": 0, "state": "not_declared", "failures": []}
        )["text_boundary_coverage"]["state"]
        == "invalid"
    )
...
def test_spine_accepts_a_report_that_bound_every_declared_allowlist(tmp_path: Path) -> None:
    coverage = _allowlist_coverage_spine(
        tmp_path, {"checked": 1, "state": "passed", "failures": []}
    )["text_boundary_coverage"]

    assert coverage["state"] == "passed"
    assert coverage["allowlist_binding_state"] == "passed"
