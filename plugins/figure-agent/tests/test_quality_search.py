...
def test_quality_search_panel_f_post_force_spacing_finish_emits_panel_block(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setattr(
        quality_search.fig_driver,
    ...
    tex_source = "\n".join(
        [
            ...
            (
                "\\draw[panelFCoulombRepulsionArrow, "
                "-{Stealth[length=7.0pt,width=4.8pt]}, "
    ...
    payload = quality_search.build_quality_search_execution(
        "fig_demo",
        goal=(
            "Panel F after force cleanup final spacing hierarchy arrow "
            "source connector air gap label polish"
        ),
        max_iterations=1,
        plugin_root=PLUGIN_ROOT,
        workspace_root=tmp_path,
    ...
    spacing_finish = [
        item
        for item in payload["candidate_set"]["candidates"]
        if item["family"] == "panel_f_post_force_spacing_finish"
    ][0]
    ...
    assert "at (9.58, 1.70) {Coulomb};" in operation["replacement"]
    assert "at (9.58, 1.42) {repulsion};" in operation["replacement"]


def test_quality_search_panel_f_post_spacing_source_finish_emits_panel_block(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setattr(
        quality_search.fig_driver,
    ...
    payload = quality_search.build_quality_search_execution(
        "fig_demo",
        goal=(
            "Panel F after spacing finish source connector apparatus electrode "
            "air gap restraint polish"
        ),
        max_iterations=1,
        plugin_root=PLUGIN_ROOT,
        workspace_root=tmp_path,
    ...
    source_finish = [
        item
        for item in payload["candidate_set"]["candidates"]
        if item["family"] == "panel_f_post_spacing_source_finish"
    ][0]
    operation = source_finish["operations"][0]
    assert source_finish["operation_scale"] == "panel_block"
...
def test_quality_search_panel_f_post_source_trap_hierarchy_emits_panel_block(
    ...
    payload = quality_search.build_quality_search_execution(
        "fig_demo",
        goal=(
            "Panel F after source finish trapped charge q_tr label hierarchy rail "
            "publication polish"
        ),
        max_iterations=1,
        plugin_root=PLUGIN_ROOT,
    ...
    trap_hierarchy = [
        item
        for item in payload["candidate_set"]["candidates"]
        if item["family"] == "panel_f_post_source_trap_hierarchy"
    ][0]
    ...
    assert "at (9.64, 3.56) {trapped charge};" in operation["replacement"]
    assert (
...
def test_quality_search_panel_f_post_gap_label_relief_emits_panel_block(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setattr(
        quality_search.fig_driver,
    ...
    payload = quality_search.build_quality_search_execution(
        "fig_demo",
        goal=(
            "Panel F after gap readability trapped charge label background "
            "leader source lead force rail polish"
        ),
        max_iterations=1,
        plugin_root=PLUGIN_ROOT,
        workspace_root=tmp_path,
    ...
    label_relief = [
        item
        for item in payload["candidate_set"]["candidates"]
        if item["family"] == "panel_f_post_gap_label_relief"
    ][0]
    operation = label_relief["operations"][0]
    assert label_relief["operation_scale"] == "panel_block"
    assert label_relief["template_id"] == "v5f_panel_f_post_gap_label_relief_v1"
    assert operation["template_id"] == "v5f_panel_f_post_gap_label_relief_v1"
    assert "quality-search F post-gap label relief" in operation["replacement"]
    assert "at (9.60, 3.62) {trapped charge};" in operation["replacement"]
    assert "(10.52, 1.02) -- (9.68, 1.02);" in operation["replacement"]
...
def test_quality_search_panel_f_trap_label_left_rail_emits_panel_block(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setattr(
        quality_search.fig_driver,
    ...
    payload = quality_search.build_quality_search_execution(
        "fig_demo",
        goal=(
            "Panel F trap label q_tr left margin label rail leader arrow "
            "cantilever trapped charge polish"
        ),
        max_iterations=1,
        plugin_root=PLUGIN_ROOT,
        workspace_root=tmp_path,
    ...
    left_rail = [
        item
        for item in payload["candidate_set"]["candidates"]
        if item["family"] == "panel_f_trap_label_left_rail"
    ][0]
    operation = left_rail["operations"][0]
    assert left_rail["operation_scale"] == "panel_block"
    assert left_rail["template_id"] == "v5f_panel_f_trap_label_left_rail_v1"
    assert operation["template_id"] == "v5f_panel_f_trap_label_left_rail_v1"
    assert "quality-search F trap label left rail" in operation["replacement"]
    assert "at (9.64, 3.70) {trapped charge};" in operation["replacement"]
