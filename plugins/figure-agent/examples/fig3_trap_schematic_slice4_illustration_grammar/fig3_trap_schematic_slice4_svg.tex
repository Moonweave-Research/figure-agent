% fig3_trap_schematic_slice4_svg.tex
% Sulfur polymer paper Fig 3 conceptual schematic — 6-panel.
% Derived from immutable Slice 3 fixture; panel e lowered through the SVG grammar backend.
...
\begin{scope}[shift={(6.00, 0.00)}]
  % The neutral grammar scene owns motif geometry only. The parent TikZ owns
  % the 3x2 composition, panel letter, legend labels, and inter-panel logic.
% figure-agent:start e.sulfur_trap_domain
  \node[inner sep=0pt, anchor=center] at (3.00, 3.10)
      {\includegraphics[width=4.30cm,height=4.30cm]{fragments/sulfur_trap_domain.pdf}};
% figure-agent:end e.sulfur_trap_domain
  \node[font=\textsf{\fontsize{6.5}{8}\selectfont}, cGray, anchor=west]
      at (0.85, 0.58) {linear chain};
  \node[font=\textsf{\fontsize{6.5}{8}\selectfont}, cGray, anchor=west]
      at (3.65, 0.58) {trap site};
\end{scope}
