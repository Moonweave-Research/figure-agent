...
\begin{scope}[shift={(4.45,0.00)}]
  ...
  \node[panelTitle,anchor=west] at (0.44,5.12) {conditional force competition};
  \node[labelMute,anchor=west] at (0.44,4.78) {floating film; illustrative condition};
  ...
  % Separate lanes record an illustrated force condition rather than an
  % independently observed force vector or a second bend snapshot.
  ...
  \node[labelMute,align=center] at (2.24,0.82)
    {illustrated reversal: $F_{\mathrm{C}}$ opposes $F_{\mathrm{M}}$;\\[-0.5pt]
...
\begin{scope}[shift={(8.90,0.00)}]
  ...
  \node[panelTitle,anchor=west] at (0.44,5.12) {schematic opposite-bend states};

  % The upper schematic state bends toward the driven electrode during the
  ...
  % After isolation and polarity reversal, the schematic cantilever bends away
