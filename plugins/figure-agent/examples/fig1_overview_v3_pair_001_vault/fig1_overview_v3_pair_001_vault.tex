% fig1_overview_v3_pair_001_vault — 2-row 7-panel narrative overview
% Pilot pair: fig1_overview_v3_pair_001 / arm: vault
% Candidate lane: non-golden overview v3, branched from accepted v2 for
...
% v3 candidate: add a minimal white backplate and slightly demote the label so
% it clears both E_C hierarchy and the Delta-E_t caliper at reduction scale.
\node[font=\sffamily\fontsize{6}{7.2}\selectfont, text=cGray!60!black,
      anchor=west, fill=white, fill opacity=0.88, text opacity=1,
      inner xsep=0.8pt, inner ysep=0.35pt] at (11.95, 8.04) {mobility edge};
...
% v3 candidate: match the mobility-edge backplate treatment so the label is
% readable as a semantic band label rather than a glyph sitting on the escape cue.
      text=cBlue!80!black, anchor=west, fill=white, fill opacity=0.88,
      text opacity=1, inner xsep=0.8pt, inner ysep=0.35pt] at (12.30, 7.64) {shallow};
\node[font=\sffamily\bfseries\fontsize{6.5}{7.8}\selectfont,
...
% v3 candidate: pull the deep escape terminus slightly left/down so it reads
...
\draw[-{Stealth[length=3.2pt,width=2.4pt]}, dashed, cRed!70!black,
      line width=0.35pt] (11.10, 6.20) .. controls (11.10, 6.60) and (11.75, 6.80) .. (11.75, 7.78);
