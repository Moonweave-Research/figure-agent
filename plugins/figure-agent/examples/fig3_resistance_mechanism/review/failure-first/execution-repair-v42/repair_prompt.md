...
- The controller will materialize a validated candidate at [examples/fig3_resistance_mechanism/review/failure-first/execution-repair-v42/repaired_generated.tex].
- Reproduce the complete bound source from [examples/fig3_resistance_mechanism/review/failure-first/execution-repair-v41/repaired_generated.tex] below.
...
- Machine finding: {"finding": "single discrete state crosses the narrow S60 peak instead of forming a label stack above it", "id": "HF024", "subject": "panel_b_s60_descriptor_peak_clearance"}
...
- Preserve the exact token [(0.965,1.30)].

## Bound editable source bytes
```tex
  \node[label, text=cBlue, font=\sffamily\bfseries\footnotesize] at (0.965,1.76) {S60};
  \node[compact label, anchor=north west] at (0.36,1.58) {single discrete state};
```
