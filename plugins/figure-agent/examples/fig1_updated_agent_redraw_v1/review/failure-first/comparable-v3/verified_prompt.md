...
- Write exactly one new source to [examples/fig1_updated_agent_redraw_v1/review/failure-first/comparable-v3/verified_generated.tex].
- Do not create an intermediate subdirectory beneath [examples/fig1_updated_agent_redraw_v1/review/failure-first/comparable-v3].
...
- Add exactly one canonical marker [% Panel A] immediately before that panel's editable body. Keep markers in narrative order.
- Add exactly one canonical marker [% Panel B] immediately before that panel's editable body. Keep markers in narrative order.
- Add exactly one canonical marker [% Panel C] immediately before that panel's editable body. Keep markers in narrative order.
- Add exactly one canonical marker [% Panel D] immediately before that panel's editable body. Keep markers in narrative order.
- Add exactly one canonical marker [% Panel E] immediately before that panel's editable body. Keep markers in narrative order.
- Add exactly one canonical marker [% Panel F] immediately before that panel's editable body. Keep markers in narrative order.
- These comments do not prescribe coordinates or layout; they bind rendered findings back to editable source regions.
