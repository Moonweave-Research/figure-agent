# Authoring Plan: fig1_hybrid_complex_panel_pilot

## Slice 3 Ownership Lock

- Source: `fig1_overview_v2_pair_001_vault` at commit `58cae332`.
- SVG/PDF fragment: complex Panel C geometry only.
- TikZ: global panel composition, all text labels, and inter-panel arrows.
- The inactive original Panel C source is retained for provenance and comparison.
- Machine gates establish reproducibility and contract compliance only;
  publication acceptance remains `not_claimed` until a human verdict exists.
...
- `design.md`
- `spec.yaml`
- `review/inherited-source-critique.md` (source history only; not a pilot verdict)
...
- Panel A linear topology is locked by `authoring_contract.md` and the inherited
  source critique; `reference/sulfur_polymer_panelA_ref.png` is an
