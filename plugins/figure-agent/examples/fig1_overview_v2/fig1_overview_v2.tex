...
% Chemistry-correct rendering: DIB benzene rings + polysulfide wavy chains
% + S₈ ring inset + transformation hint.
% ============================================================

% --- Soft amber radial wash (centered on triangle centroid for balance) ---
\fill[cAmber!10] (1.62, 6.90) circle (1.95cm);
\fill[cAmber!06] (1.62, 6.90) circle (1.45cm);

% Phase 4 prototype — 3 DIB benzene rings in triangular layout.
% Each DIB: hexagonal aromatic ring + 2 polysulfide chains at varying angles
% to suggest amorphous network branching.

% --- Helper: DIB benzene ring at origin (used inside scope shift) ---
% Bumped to radius 0.42cm (was 0.35) + slightly thicker aromatic notation.
\newcommand{\dibRing}{
...
% --- Helper: polysulfide chain (angle, label_x, label, end_radius) ---
% #4 = chain end radius (varies 0.85-1.20 for organic asymmetry)
% Solid = 0.50..(end-0.30), Faded tail = (end-0.30)..end
% \polysulfide{angle}{label_x}{label}{end_radius}{amplitude}
\newcommand{\polysulfide}[5]{
  \begin{scope}[rotate=#1]
    \pgfmathsetmacro{\fadeStart}{#4 - 0.30}
    % Isopropenyl linker
    \draw[cAmber!85!black, line width=0.85pt] (0.42, 0) -- (0.58, 0);
    % Wavy main solid (amplitude #5)
    \draw[cAmber!85!black, line width=0.85pt, smooth, samples=60,
          domain=0.58:\fadeStart]
          plot (\x, {#5*sin((\x-0.58)*900)});
    % Wavy faded tail
    \draw[cAmber!85!black, line width=0.85pt, smooth, samples=50,
          domain=\fadeStart:#4, opacity=0.25]
          plot (\x, {#5*sin((\x-0.58)*900)});
    \node[font=\sffamily\fontsize{5.5}{6.6}\selectfont,
          text=cAmber!85!black]
      at (#2, 0.18) {#3};
  \end{scope}
}

% DIB1 — top-center (centered between DIB2 and DIB3 for equilateral triangle)
\begin{scope}[shift={(1.62, 7.90)}]
  \dibRing
  \polysulfide{210}{0.65}{S}{1.10}{0.09}                 % toward DIB2
  \polysulfide{330}{0.70}{S\,--\,S}{1.20}{0.10}          % toward DIB3 (wider wave)
\end{scope}

% DIB2 — bottom-left
\begin{scope}[shift={(0.85, 6.40)}]
  \dibRing
  \polysulfide{30}{0.65}{S\,--\,S\,--\,S}{1.05}{0.08}    % toward DIB1
  \polysulfide{180}{0.62}{S}{0.85}{0.07}                 % short terminal (narrower wave)
\end{scope}

% DIB3 — bottom-right
\begin{scope}[shift={(2.40, 6.40)}]
  \dibRing
  \polysulfide{150}{0.65}{S\,--\,S}{1.10}{0.10}          % toward DIB1
  \polysulfide{0}{0.62}{S}{0.85}{0.07}                   % short terminal
\end{scope}

% S₈ ring inset — upper-right corner of Panel A (bumped for prominence)
\begin{scope}[shift={(3.00, 8.55)}]
  \pgfmathsetmacro{\sR}{0.40}  % S₈ ring radius (bumped 0.32→0.40)
  % Octagonal bond polygon
  \foreach \i in {0,...,7} {
  ...
  % S labels at each vertex (bigger, white-fill so ring lines don't cross)
  \foreach \i in {0,...,7} {
    \pgfmathsetmacro{\sang}{90 - \i*45}
...
% Linear polysulfide notation (S-S-S) — placed inside Panel A bounds (left of ring)
\node[font=\sffamily\fontsize{6.5}{7.8}\selectfont,
      text=cAmber!85!black, anchor=east] at (2.55, 8.55) {S\,–\,S\,–\,S};

% Transformation arrow — S₈ ring → polymer network area
% Stronger line width + arrow head; lands near DIB1 upper-right edge.
\draw[cAmber!85!black, dashed, line width=0.90pt,
      -{Stealth[length=5pt,width=4pt]}]
  (2.85, 8.20) to[out=-110, in=35] (1.95, 7.70);

% --- Caption (placeholder during prototype phase) ---
