...
% iter 51: Debye label was baseline-on-axis (y=0.40 = axis y) — Gemini Defect
% (re-review) HIGH. Leader endpoint shortened to y=0.42 so it stays above
% axis line; label anchor=north west at y=0.38 sits cleanly below axis.
\draw[cGray!55!black, line width=0.30pt] (2.50, 0.55) -- (2.70, 0.42);
\node[labelMute, text=cGray!65!black, anchor=north west, inner sep=1pt]
  at (2.70, 0.38) {Debye};

% D-4 main equation label
\node[labelStrong, anchor=west] at (0.55, 2.75) {$I(t)\!\sim\!t^{-n}$};

% D-7b curve ID labels — iter 51: add fill=white to break the curve under
% each label (Gemini Defect 1: HIGH label-on-line clash on deep-rich /
% shallow-rich at curve y-positions 1.77 / 1.09).
\node[labelMute, anchor=south, text=cRed!75!black,
      fill=white, inner sep=1pt] at (2.20, 1.75) {deep-rich};
\node[labelMute, anchor=south, text=cBlue!75!black,
      fill=white, inner sep=1pt] at (2.50, 1.05) {shallow-rich};
