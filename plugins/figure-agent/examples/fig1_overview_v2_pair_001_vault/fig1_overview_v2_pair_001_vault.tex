...
% iter 56: RED line width 0.80 → 0.90pt (primary narrative tier per briefing
% line-weight hierarchy: primary 0.9pt = narrative paths). Sulfur polymer is
% paper hero = narrative line. BLUE stays 0.80pt (control = annotation tier).
% iter 60 illustration polish: line cap=round (soft endpoints vs default butt).
\draw[cRed!80!black, line width=0.90pt, line cap=round]
  (0.65, 2.30) -- (3.85, 0.55);
\draw[cBlue!80!black, line width=0.80pt, line cap=round]
  (0.65, 2.30) -- (3.85, 1.50);
...
% D-4 main equation label — iter 62: moved to plot interior far-right (3.00, 2.40)
% to clear "low n" label region. iter 63: \mathsf trial REVERTED — caused
% inconsistency with Panel E/F math labels ($V_s(t)$, $g(E_t)$, $\tau_d$,
% $F_{\mathrm{Maxwell}}$) all using default Computer Modern serif italic.
% Figure-wide math convention = serif italic; text labels = sans-serif. Mixed
% register is journal-standard.
\node[labelStrong, anchor=west] at (3.00, 2.40)
  {$I(t)\!\sim\!t^{-n}$};
...
\path[draw=none] (0.65, 2.30) -- (3.85, 1.50)
  node[pos=0.40, labelMute, text=cBlue!75!black, sloped, above=4pt]
  {low $n$};
