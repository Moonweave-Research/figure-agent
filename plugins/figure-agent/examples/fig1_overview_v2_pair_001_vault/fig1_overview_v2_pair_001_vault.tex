...
% A-2 v8.4 [briefing relax R2, 2026-05-26]: terminal danglings get
% ball-shaded atom emphasis — "linear copolymer terminus" cue binding
% to bivalent-DIB topology constraint (§9 line 396 LOCKED). B-1 endpoint
% recipe (ball color + thin outline, radius 0.035cm).
\foreach \tx/\ty in {0.10/7.20, 3.50/6.50} {
  \shade[ball color=cAmber!85!black] (\tx, \ty) circle (0.035cm);
  \draw[cAmber!95!black, line width=0.16pt] (\tx, \ty) circle (0.035cm);
}
...
% A-5 v8.5 [briefing relax R2, 2026-05-26]: v8.4 dial turn-up per advisor.
% shadow opacity 0.18→0.40, offset 0.05→0.08cm; gradient delta widened
% (cAmberSphere!18→cAmber!70); specular opacity 0.32→0.65, radius 0.6→1.5mm.
% Chemistry constraint (regular octagon + 8 'S' + center S₈) preserved.
\node[regular polygon, regular polygon sides=8, fill=black, opacity=0.40,
      minimum size=7mm, inner sep=0pt, draw=none]
   at (3.13,8.47) {};
\node[regular polygon, regular polygon sides=8, draw=cAmber!80!black,
      line width=0.7pt, minimum size=7mm,
      top color=cAmberSphere!10, bottom color=cAmber!70, shading angle=135,
      inner sep=0pt] (s8) at (3.05,8.55) {};
\fill[white, opacity=0.65]
  ([xshift=-1.5mm, yshift=1.5mm]s8.center) circle (1.5mm);
...
% empty space.
% A-6 v8.4 [briefing relax R2, 2026-05-26]: arrow tone cAmber!70!black
% → cAmber!85!black to match polysulfide chain stroke (transformation
% result species). Visually binds arrow to "becomes this color" semantic.
% Stealth tip, dashed (§17 transformation), 0.55pt, bend, target vertex
% all preserved.
\draw[-{Stealth[length=4pt,width=3pt]}, dashed, cAmber!85!black,
...
\fill[black, opacity=0.35] (7.62, 6.13) rectangle (9.92, 7.63);
...
% v8.5: NW corner ellipsoidal specular spot — turned up per advisor
\fill[white, opacity=0.65] (7.90, 7.48) ellipse (3.0mm and 1.0mm);
% Right-edge thin shadow accent (depth cue) — turned up
\draw[cAmber!85!black, line width=0.55pt, opacity=0.75]
  (9.85, 6.22) -- (9.85, 7.68);
% v8.5: bottom-edge ambient occlusion turned up
\draw[cAmber!90!black, line width=0.55pt, opacity=0.55]
  (7.60, 6.26) -- (9.80, 6.26);
...
\foreach \site in {siteS1, siteS2} {
  ...
  \draw[cBlue!95!black, line width=0.20pt] (\site) circle (0.07);
}
\foreach \site in {siteD1, siteD2} {
  \shade[ball color=cRed!70!black] (\site) circle (0.075);
  \draw[cRed!95!black, line width=0.22pt] (\site) circle (0.075);
}
...
\node[font=\sffamily\fontsize{6.5}{7.8}\selectfont, text=cGray!65!black,
      anchor=west] at (12.15, 7.85) {mobility edge};
...
\draw[cRed!80!black, line width=0.65pt]
  plot[domain=5.40:6.70, samples=30, smooth]
    ({10.55 + 0.55*exp(-(\x-6.05)*(\x-6.05)/0.0648)}, \x);
% C-R2/C-R3 v8.5 [briefing relax R2, 2026-05-26]: dial turn-up. spec
% opacity 0.40→0.70, radius 0.4mm→1.1mm (~3x).
\fill[white, opacity=0.70] (11.00, 7.49) circle (1.1mm);
\fill[white, opacity=0.70] (11.10, 6.05) circle (1.1mm);
...
\node[font=\sffamily\fontsize{6}{7.2}\selectfont, text=cGray!80!black,
      anchor=south, rotate=90] at (10.35, 6.30) {Energy};
...
\foreach \tY in {7.55, 7.35} {
  \draw[cBlue!80!black, line width=1.15pt, line cap=round] (10.95, \tY) -- (11.55, \tY);
}
\foreach \tY in {6.20, 5.85} {
  \draw[cRed!80!black, line width=1.15pt, line cap=round] (10.95, \tY) -- (11.55, \tY);
}
...
\foreach \dx/\dy in {11.15/7.55, 11.35/7.35} {
  ...
  \draw[cBlue!95!black, line width=0.20pt] (\dx, \dy) circle (0.075);
}
\foreach \dx/\dy in {11.15/6.20, 11.35/5.85} {
  \shade[ball color=cRed!70!black] (\dx, \dy) circle (0.08);
  \draw[cRed!95!black, line width=0.22pt] (\dx, \dy) circle (0.08);
}
...
\draw[{Stealth[length=2.6pt,width=2pt]}-{Stealth[length=2.6pt,width=2pt]},
      cRed!80!black, line width=0.45pt] (13.40, 8.27) -- (13.40, 5.83);
...
\node[anchor=south,
      font=\sffamily\bfseries\fontsize{7.5}{9}\selectfont,
      text=cGray!85!black] at (10.40, 8.92) {localized traps};
...
% ============================================================
% 2026-05-22 NC-Fig-1 redirect: caption backdrop fill REMOVED. On clean
% white background, bold 7.5pt sans cGray!90 emerges from white via weight
% alone. No backdrop punch (would create a wash-shaped object on otherwise
% white figure — NC anti-pattern).
\node[font=\sffamily\bfseries\fontsize{7.5}{9}\selectfont,
      text=cGray!90!black, anchor=south, align=center, inner sep=2pt]
  at (7.00, 4.92) {convergent evidence};

% Branching root (in Panel C bottom band, x=6.95 = Panel C left + slight inset)
\coordinate (branchRoot) at (6.95, 4.85);
% v8.4 [briefing relax R2, 2026-05-26]: small anchor dot at branchRoot —
% "3 paths from one origin" visual cue binding to 'convergent evidence'
% caption semantic. Sub-pixel (0.6pt radius), tone-matched to spoke stroke.
\fill[cGray!75!black] (branchRoot) circle (0.025);

% 3 spokes — narrative weight history: 0.9pt→1.1pt promotion (C403) made the
% fan visually dominant, but at zoom the heavy Stealth heads + cGray!80 stroke
% read as PowerPoint not Nature Communications. Designer iter (2026-05-22):
% slim down to 0.7pt cGray!55 with 4.5pt Stealth heads — still on top of the
% 3-tier (annotation 0.7pt, secondary 0.55pt, axes 0.55pt) but lighter and
% closer to NC main-text figure schematic convention.
\draw[-{Stealth[length=4.5pt,width=3pt]}, cGray!55!black, line width=0.7pt]
  (branchRoot) -- (2.28, 4.30);
\draw[-{Stealth[length=4.5pt,width=3pt]}, cGray!55!black, line width=0.7pt]
  (branchRoot) -- (7.05, 4.50);
\draw[-{Stealth[length=4.5pt,width=3pt]}, cGray!55!black, line width=0.7pt]
  (branchRoot) -- (12.225, 4.30);

% Spoke modality labels — 2026-05-22 NC redirect: cAmber!8 fill → white fill.
% Same "punch the arrow under label letters" purpose, now using plain white
% rectangle behind label (NC main-text convention: any backdrop is white,
% no tint). Arrow-on-label crossing is still broken; no colored box on figure.
\node[font=\sffamily\itshape\fontsize{6}{7.2}\selectfont, text=cGray!75!black,
      anchor=south, fill=white, text opacity=1, inner sep=1pt]
...
\draw[cGray!30, line width=0.35pt, rounded corners=2pt]
  (0.05, 0.10) rectangle (13.85, 4.50);
\draw[cGray!30, line width=0.35pt] (4.62, 0.10) -- (4.62, 4.50);
\draw[cGray!30, line width=0.35pt] (9.47, 0.10) -- (9.47, 4.50);
...
% D apparatus v8.5 [briefing relax R2, 2026-05-26]: dial turn-up.
% shadow 0.16→0.40, offset 0.02→0.05cm; gradient delta widened
% cGray!4→cGray!40; specular 0.35→0.65, 0.5mm→1.2mm.
\fill[black, opacity=0.40] (0.55, 3.02) rectangle (1.35, 3.72);
\shade[top color=cGray!4, bottom color=cGray!40, rounded corners=1pt]
  (0.50, 3.07) rectangle (1.30, 3.77);
\draw[cGray!75!black, line width=0.30pt, rounded corners=1pt]
  (0.50, 3.07) rectangle (1.30, 3.77);
\fill[white, opacity=0.65] (0.62, 3.70) circle (1.2mm);
\node[font=\sffamily\itshape\fontsize{6}{7.2}\selectfont, text=cGray!75!black,
      anchor=center] at (0.90, 3.57) {SMU};
\node[font=\sffamily\fontsize{6}{7.2}\selectfont, text=cGray!75!black,
      anchor=center] at (0.90, 3.27) {V\,/\,A};
...
\shade[top color=cGray!8, bottom color=cGray!70]
...
% Cycle 2 / C405 (2026-05-22): clearance above=4pt → 6pt + pos 0.92→0.88 so
% label sits clearly ABOVE asymptotic tail rather than grazing it at thumbnail.
% Iter 21 (2026-05-23): Debye label at pos=0.88 above=6pt had its 'y'
% descender grazing the red high-n power-law line just above; iter 21a
% below=3pt put the label on the x-axis. Compromise: lower pos to 0.70
% where Debye dashed is at y≈0.85 and the red high-n line is at y≈1.2
% (gap ~0.35 cm), with above=4pt — label sits in the middle of the gap,
% clearly anchored to Debye curve and well clear of the red line.
\path[draw=none] (0.65, 2.30) .. controls (0.95, 1.40) and (2.00, 0.50) .. (3.40, 0.45)
  node[pos=0.70, labelMute, text=cGray!65!black, sloped, above=4pt, inner sep=1pt]
  {Debye};
...
\path[draw=none] (0.65, 2.30) -- (3.85, 0.55)
  node[pos=0.55, font=\sffamily\bfseries\itshape\fontsize{6.5}{7.8}\selectfont,
       text=cRed!90!black, sloped, above=6pt, inner sep=1pt]
  {high $n$};
\path[draw=none] (0.65, 2.30) -- (3.85, 1.50)
  node[pos=0.40, font=\sffamily\bfseries\itshape\fontsize{6.5}{7.8}\selectfont,
       text=cBlue!60!black, sloped, above=6pt, inner sep=1pt]
  {low $n$};
...
\fill[black, opacity=0.16] (6.81, 3.59) rectangle (7.22, 3.6);
...
% Iter 12: subtle horizontal striations on substrate (suggesting polished
% metal / ITO conductor surface, briefing §13.6 ref01 Checa NatComm 2023).
% Two thin lines at low opacity — barely visible, adds material weight
% without competing with sample-charges or label leader.
\foreach \sy in {3.22, 3.30, 3.38} {
  \draw[white, opacity=0.25, line width=0.18pt] (5.95, \sy) -- (7.40, \sy);
}
...
\draw[cGray!75!black, line width=0.35pt, rounded corners=1pt]
  (7.45, 3.3) -- (7.55, 3.3) -- (7.55, 3.1);
\draw[cGray!75!black, line width=0.42pt] (7.48, 3.1) -- (7.62, 3.1);
\draw[cGray!75!black, line width=0.36pt] (7.5, 3.07) -- (7.6, 3.07);
\draw[cGray!75!black, line width=0.32pt] (7.52, 3.04) -- (7.58, 3.04);
...
% E apparatus v8.5 [briefing relax R2, 2026-05-26]: HV+ box dial turn-up.
% shadow 0.16→0.40 + offset; gradient delta wider cGray!4→cGray!40;
% spec 0.35→0.65, 0.4mm→1.0mm.
\fill[black, opacity=0.40] (5.74, 4.05) rectangle (6.59, 4.35);
\shade[top color=cGray!4, bottom color=cGray!40, rounded corners=0.4pt]
  (5.7, 4.1) rectangle (6.55, 4.4);
\draw[cGray!55!black, line width=0.20pt, rounded corners=0.4pt]
  (5.7, 4.1) rectangle (6.55, 4.4);
\fill[white, opacity=0.65] (5.79, 4.35) circle (1.0mm);
% Display — iter 50: enlarged vertically 0.07→0.16cm (was squashed 9:1 flat
% per user "신호 찌그러짐"). New extent 4.22..4.38.
\fill[cGray!70!black, rounded corners=0.2pt] (5.8, 4.22) rectangle (6.45, 4.38);
\draw[cGray!85!black, line width=0.12pt, rounded corners=0.2pt]
  (5.8, 4.22) rectangle (6.45, 4.38);
...
\draw[cAmber!90!white, line width=0.35pt] (5.88, 4.34) -- (6.38, 4.34);
\draw[cAmber!90!white, line width=0.35pt] (6, 4.28) -- (6.26, 4.28);
...
\fill[cGray!70!black] (6.15, 4.12) circle (0.018);
\fill[cGray!40!black] (6.15, 4.12) circle (0.009);
% Wire from terminal (now inside box at 4.12) to needle cuff at 4.05.
\draw[cGray!75!black, line width=0.32pt] (6.15, 4.1) -- (6.15, 4.05);
...
\shade[left color=cGray!75!black, right color=cGray!90!black]
  (6.135, 4.05) -- (6.165, 4.05) -- (6.15, 3.73) -- cycle;
...
\fill[cGray!65!black] (6.15, 4.05) circle (0.025);
...
\fill[cRed!22, opacity=0.55] (6.15, 3.73) -- (5.95, 3.6) -- (6.55, 3.6) -- cycle;
\draw[cRed!55, line width=0.35pt, opacity=0.65] (6.15, 3.73) -- (6.15, 3.6);
...
\foreach \cx in {6.25, 6.60, 6.95, 7.30} {
  \shade[ball color=cRed!70!black] (\cx, 3.62) circle (0.045);
...
\draw[cGray!80!black, line width=0.18pt]
  (7.04, 3.87) -- (7.04, 4.05);
\fill[white, opacity=0.55] (6.995, 3.87) rectangle (7.005, 4.05);
...
\draw[cGray!75!black, line width=0.45pt] (6.8, 3.8) rectangle (7.2, 3.87);
\fill[white, opacity=0.55] (6.81, 3.868) rectangle (7.18, 3.87);
...
\node[font=\sffamily\fontsize{6.5}{7.8}\selectfont, text=cGray!65!black,
% Iter 17: pdftotext bbox showed V_s probe label glyph top at TikZ y=4.56,
% 0.06 cm above the Row 2 box outline (y=4.50). Shifted anchor y 4.10 →
% 4.00 so glyph top lands at ~4.46 with a clear 0.04 cm margin from box
% top. Label bottom moves to y≈4.18, still 0.13 cm above probe shaft top.
      anchor=south] at (7.1, 4.0) {$V_s$ probe};
...
% E apparatus v8.5 [briefing relax R2, 2026-05-26]: V_s meter dial turn-up.
\fill[black, opacity=0.40] (7.72, 3.53) rectangle (8.72, 4.00);
\shade[top color=cGray!4, bottom color=cGray!40, rounded corners=0.4pt]
  (7.68, 3.58) rectangle (8.68, 4.05);
\draw[cGray!55!black, line width=0.20pt, rounded corners=0.4pt]
  (7.68, 3.58) rectangle (8.68, 4.05);
\fill[white, opacity=0.65] (7.77, 4.00) circle (1.0mm);
...
\fill[cGray!70!black, rounded corners=0.3pt] (7.72, 3.78) rectangle (8.56, 4);
\draw[cGray!85!black, line width=0.15pt, rounded corners=0.3pt]
  (7.72, 3.78) rectangle (8.56, 4);
...
\draw[cGray!75!black, line width=0.30pt]
  (7, 4.05) .. controls (7.15, 4.05) and (7.52, 3.9) .. (7.72, 3.85);
\fill[cGray!85!black] (7.72, 3.85) circle (0.024);
\fill[cGray!50!black] (7.72, 3.85) circle (0.012);
...
\node[font=\sffamily\itshape\fontsize{6.5}{7.8}\selectfont,
      text=cGray!65!black, anchor=west] at (4.72, 3.5) {polymer};
\draw[cGray!55!black, line width=0.18pt] (5.45, 3.5) -- (5.88, 3.55);
\fill[cGray!55!black] (5.88, 3.55) circle (0.012);
...
\draw[cRed!85, fill=cRed!25, line width=0.55pt]
  (6.48, 0.4) .. controls (6.66, 0.4) and (6.66, 1.15) .. (6.98, 1.15)
              .. controls (7.3, 1.15) and (7.3, 0.4) .. (7.48, 0.4);
...
\foreach \mx/\my in {6.75/0.8, 6.98/1.15, 7.21/0.8} {
...
\draw[cGray!65!black, line width=0.22pt] (5.9, 1.42) -- (7, 1.42);
\draw[cGray!65!black, line width=0.22pt] (5.9, 1.4) -- (5.9, 1.44);
\draw[cGray!65!black, line width=0.22pt] (7, 1.4) -- (7, 1.44);
...
\node[labelMute, anchor=north, text=cGray!70!black,
      font=\sffamily\fontsize{6.5}{7.8}\selectfont] at (5.9, 0.32) {Shallow};
\node[labelMute, anchor=north, text=cGray!70!black,
      font=\sffamily\fontsize{6.5}{7.8}\selectfont] at (6.98, 0.32) {Deep};
...
% F apparatus v8.5 [briefing relax R2, 2026-05-26]: PSU dial turn-up.
\fill[black, opacity=0.40] (11.84, 3.15) rectangle (12.69, 3.80);
\shade[top color=cGray!2, bottom color=cGray!45]
  (11.8, 3.2) rectangle (12.65, 3.85);
\draw[cGray!60!black, line width=0.25pt]
  (11.8, 3.2) rectangle (12.65, 3.85);
\fill[white, opacity=0.65] (11.89, 3.81) circle (1.0mm);
...
% F-4 v8.5 [briefing relax R2, 2026-05-26]: dial turn-up. shadow 0.18→0.40,
% offset bigger; gradient delta widened cGray!10→cGray!75; bevel 0.55→0.85.
\fill[black, opacity=0.40] (11.89, 2.38) rectangle (12.29, 2.48);
\shade[top color=cGray!10!white, bottom color=cGray!75!white]
  (11.85, 2.42) rectangle (12.25, 2.52);
\draw[cGray!75!black, line width=0.40pt]
  (11.85, 2.42) rectangle (12.25, 2.52);
\draw[white, opacity=0.85, line width=0.50pt] (11.87, 2.515) -- (12.23, 2.515);
\draw[cGray!50!black, line width=0.40pt] (11.75, 2.65) -- (12.35, 2.65);
\foreach \dx in {0,0.10,0.20,0.30,0.40,0.50} {
  \draw[cGray!50!black, line width=0.25pt]
    ({11.75+\dx}, 2.65) -- ({11.70+\dx}, 2.72);
}
\draw[cGray!75!black, line width=0.40pt] (12.05, 2.52) -- (12.05, 2.65);
...
\shade[top color=cAmber!22, bottom color=cAmber!42, rounded corners=0.3mm]
  (11.92, 2.42) .. controls (11.92, 1.85) and (11.65, 1.35) ..
  (11.35, 0.85) -- (11.5, 0.85) .. controls (11.8, 1.35) and
  (12.05, 1.85) .. (12.05, 2.42) -- cycle;
\draw[cAmber!80!black, line width=0.55pt, rounded corners=0.3mm]
  (11.92, 2.42) .. controls (11.92, 1.85) and (11.65, 1.35) ..
  (11.35, 0.85) -- (11.5, 0.85) .. controls (11.8, 1.35) and
  (12.05, 1.85) .. (12.05, 2.42) -- cycle;
\draw[cAmber!65!black, line width=0.28pt, opacity=0.38]
  (11.855, 2.35) .. controls (11.855, 1.85) and (11.585, 1.35) .. (11.285, 0.88);
\draw[cAmber!65!black, line width=0.28pt, opacity=0.38]
  (11.985, 2.35) .. controls (11.985, 1.85) and (11.715, 1.35) .. (11.415, 0.88);
% F-5 v8.5 [briefing relax R2, 2026-05-26]: dial turn-up. spec opacity
% 0.30→0.60, radius 0.4mm→1.2mm (3x). Wet polymer cue prominent.
\fill[white, opacity=0.60] (11.965, 2.28) circle (1.2mm);
...
\foreach \cx/\cy in {11.85/2.00, 11.75/1.55, 11.60/1.10} {
...
\node[labelMute, anchor=north east, font=\sffamily\fontsize{6.5}{7.8}\selectfont,
      text=cRed!75!black] at (10.85, 1.27) {repulsion};
...
% F-8 v8.5 [briefing relax R2, 2026-05-26]: dial turn-up. shadow 0.18→0.45,
% offset 0.03→0.06cm; gradient delta wider cGray!90→cGray!25; left stripe
% 0.50→0.85, thicker; NW spec 0.45→0.70 + 0.6mm→1.3mm.
\fill[black, opacity=0.45] (13.29, 0.34) rectangle (13.46, 2.54);
\shade[left color=cGray!90!black, right color=cGray!25!black]
  (13.23, 0.4) rectangle (13.4, 2.6);
\draw[cGray!85!black, line width=0.50pt]
  (13.23, 0.4) rectangle (13.4, 2.6);
\draw[white, opacity=0.85, line width=0.55pt]
  (13.235, 0.45) -- (13.235, 2.55);
\fill[white, opacity=0.70] (13.255, 2.53) circle (1.3mm);
...
\node[font=\sffamily\itshape\fontsize{6.5}{7.8}\selectfont, text=cGray!65!black,
      anchor=north] at (12.45, 1.72) {$F_{\mathrm{Maxwell}}$};
