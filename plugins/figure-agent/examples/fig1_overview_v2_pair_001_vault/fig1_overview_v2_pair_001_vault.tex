...
% Now: D + F down-verticals (grouping) + horizontal bracket + center up-arrow.
\draw[cGray!55, line width=0.35pt] (2.275, 4.85) -- (2.275, 4.55);
\draw[cGray!55, line width=0.35pt] (11.70, 4.85) -- (11.70, 4.55);
\draw[cGray!55, line width=0.35pt] (2.275, 4.85) -- (11.70, 4.85);
% Convergent arrow: up from bracket center pointing TO caption + Panel C.
% AGENT+ADVISOR FIX (2026-05-28): agents found convergence reads weak because
% up arrow was 0.35pt = same weight as grouping bracket/verticals (no hierarchy).
% rowBridgeArrow style (line 40: 10pt head, 1.6pt) is too heavy for this 0.27cm
% arrow (head would exceed shaft), so applied PROPORTIONATE heavy treatment:
% weight 0.35→0.70pt + head 3→5pt + tone cGray!55→cGray!70!black + length
% extended (tip 5.02→5.12). Now the convergence VECTOR is distinctly heavier
% than the thin (0.35pt) grouping bracket — visual hierarchy: thin bracket =
% "3 evidence grouped", bold up arrow = "converge on the model".
\draw[-{Stealth[length=5pt,width=3.5pt]}, cGray!70!black, line width=0.70pt]
  (6.975, 4.85) -- (6.975, 5.12);
...
% AGENT+ADVISOR FIX: caption y 5.05 → 5.15 so heavier up-arrow tip (now 5.12)
% sits just below caption baseline (0.03cm gap) instead of merging into glyphs.
% Caption top 5.29; Sulfur axis label bottom ~5.40 → 0.11cm gap (still clear).
\node[font=\sffamily\itshape\fontsize{6}{7.2}\selectfont, text=cGray!60!black,
      anchor=south, inner sep=0pt] at (6.975, 5.15) {convergent evidence};
% Modality labels: connector tier 6pt cGray!70 in inter-row gap.
% kinetic + mechanical sit below their down-verticals (y=4.53 anchor=north).
% AGENT+ADVISOR FIX: ISPD moved UP to (6.975, 4.62) anchor=south — now in the
% gap between bracket horizontal (4.85) and Panel E frame top (4.50), above the
% removed down-vertical. Glyph 4.62..4.80; "V_s probe" at 4.0..4.19 → 0.43cm
% clear (was 0.13cm collision). ISPD is the convergence-center column so a
% slightly higher label sits naturally under the up-arrow origin.
\node[font=\sffamily\fontsize{6}{7.2}\selectfont, text=cGray!70!black,
      anchor=north] at (2.275, 4.53) {kinetic};
\node[font=\sffamily\fontsize{6}{7.2}\selectfont, text=cGray!70!black,
      anchor=south] at (6.975, 4.62) {ISPD};
\node[font=\sffamily\fontsize{6}{7.2}\selectfont, text=cGray!70!black,
