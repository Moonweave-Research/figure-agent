...
\fill[cAmber!28] (1.70, 3.25) rectangle (3.50, 3.70);
...
% accordingly (Gemini "ground lead cramped compared to SMU side leads").
% SMU leads have 0.20cm horizontal segment; now ground matches that breathing
% room.
\draw[cGray!75!black, line width=0.28pt] (3.50, 3.20) -- (3.70, 3.20) -- (3.70, 3.06);
...
\draw[cGray!85!black, dashed, line width=0.75pt]
  (0.75, 2.45) .. controls (1.15, 2.30) and (1.55, 1.05) .. (2.55, 0.56);
% iter 53: Debye label y 0.45 → 0.55 (Gemini HIGH "too close to x-axis"). +0.10cm
% (~3pt) up. At anchor=west (2.60, 0.55), text top ~0.73. shallow-rich curve at
% x=2.60..3.10 spans y=1.05..0.83 — label top stays ≥0.10cm clear at worst x.
% Adjacent to truncated dashed endpoint (2.55, 0.56) → eye traces curve to label.
...
% D-7b curve ID labels — iter 52d: switched from anchor=south/center to
% `sloped` path-attached label (TikZ journal convention). Labels rotate to
% match curve slope and sit `above=2pt` perpendicular to curve — guaranteed
% non-overlap with the line they label (Gemini Panel D iter 52b/c HIGH:
% horizontal labels visually touched blue curve despite 0.15-0.18cm math
% clearance). Path is `draw=none` since the actual curves D-5/D-6 are drawn
% separately above with their own line widths.
\path[draw=none] (0.65, 2.40) -- (3.85, 1.10)
  node[midway, labelMute, text=cRed!75!black, sloped, above=3pt]
  {deep-rich};
% iter 52e: shallow-rich perpendicular offset above=2pt → above=4pt (Gemini
% MEDIUM: dashed Debye crossed underneath letters at original 2pt offset).
% Increased separation pushes label clear of Debye descent zone without
% needing fill=white mask (which would break the parallel blue solid curve).
\path[draw=none] (0.65, 1.90) -- (3.85, 0.50)
  node[midway, labelMute, text=cBlue!75!black, sloped, above=4pt]
  {shallow-rich};
...
% Polymer film — iter 16: height 0.07 → 0.15cm (y=3.45..3.60) for visible
% material layer. Hairline visibility issue resolved. Aspect 22:1 → 10:1.
\shade[top color=cAmber!38, bottom color=cAmber!20]
...
\shade[top color=cGray!42, bottom color=cGray!22]
...
% Ground wire + bars — iter 45 (Pri 5b): line widths bumped for print visibility.
\draw[cGray!75!black, line width=0.35pt] (7.85, 3.30) -- (7.95, 3.30) -- (7.95, 3.10);
...
\node[font=\sffamily\bfseries\fontsize{5}{6}\selectfont, text=cGray!88!black]
  at (6.55, 4.14) {HV+};
...
% Sparks — iter 45 (Pri 4): softened from rigid solid spikes to subtle corona
% discharge feel: opacity 0.6 + thinner 0.35pt + lighter cRed!55.
\draw[cRed!55, line width=0.35pt, opacity=0.65] (6.55, 3.73) -- (6.45, 3.60);
...
\draw[cRed!55, line width=0.35pt, opacity=0.65] (6.55, 3.73) -- (6.65, 3.60);
% Surface charges — iter 48 (Codex): 5→4 charges (drop leftmost under spark),
% radius 0.045→0.038 (Codex "shrink").
\foreach \cx in {6.65, 7.00, 7.35, 7.70} {
  \fill[cRed!75!black] (\cx, 3.62) circle (0.038);
...
%
% Vertical shaft — iter 40: bottom 3.77→3.87 (matches lifted disk top). Top
% stays 3.95 (cable bezier start). Shaft length 0.18→0.08cm — shorter.
\shade[left color=cGray!80!black, middle color=cGray!18,
       right color=cGray!80!black]
  (7.36, 3.87) rectangle (7.44, 3.95);
\draw[cGray!80!black, line width=0.18pt]
  (7.36, 3.87) -- (7.36, 3.95);
\draw[cGray!80!black, line width=0.18pt]
  (7.44, 3.87) -- (7.44, 3.95);
\fill[white, opacity=0.55] (7.395, 3.87) rectangle (7.405, 3.95);
% Disk electrode — iter 40: lifted 0.10cm (3.70..3.77 → 3.80..3.87) per user
% "probe too close to sample". Air gap from polymer (3.60) now 0.20cm (was 0.10).
\shade[left color=cAmber!28, middle color=cAmber!65,
       right color=cAmber!22]
  (7.20, 3.80) rectangle (7.60, 3.87);
\draw[cAmber!75!black, line width=0.45pt] (7.20, 3.80) rectangle (7.60, 3.87);
\fill[white, opacity=0.55] (7.21, 3.868) rectangle (7.58, 3.870);
% Vibration cue — iter 50: x 7.55→7.62 to clear bezier cable (was sharing
% x=7.55 with cable control point — visual collision).
\node[font=\fontsize{9}{10.8}\selectfont, text=cGray!75!black,
      inner sep=0pt] at (7.62, 3.91) {$\updownarrow$};
% Air gap d — iter 49 (Codex): shifted right 7.72→7.80 to open probe-d-meter
% cluster (Codex "probe cluster cramped").
\draw[cGray!55!black, dashed, line width=0.18pt] (7.80, 3.78) -- (7.80, 3.62);
\draw[cGray!55!black, line width=0.22pt] (7.77, 3.78) -- (7.83, 3.78);
\draw[cGray!55!black, line width=0.22pt] (7.77, 3.62) -- (7.83, 3.62);
\node[font=\sffamily\itshape\fontsize{5}{6}\selectfont, text=cGray!55!black,
      anchor=west] at (7.83, 3.70) {$d$};
...
% iter 51: thin space between V_s and meter (Gemini Defect 3 MEDIUM: subscript
% s kerning against "m" — `\,` inserts a 3mu thin space).
% iter 51 fix-up: unify font (V was math-sans `$\mathsf{V_s}$` while "meter"
% was text-sans-bold; mismatch flagged by Gemini re-review #4). New form uses
% text V (inherits surrounding bold-sans) + math-mode subscript only.
\node[font=\sffamily\bfseries\fontsize{5.5}{6.6}\selectfont, text=cGray!88!black]
  at (8.54, 3.66) {V$_{\mathsf{s}}$\,meter};
...
% Cable + Input port — iter 48 (Codex): box left wall 8.00→8.08, port inside at 8.12.
\draw[cGray!75!black, line width=0.22pt]
  (7.40, 3.95) .. controls (7.55, 3.95) and (7.92, 3.85) .. (8.12, 3.85);
...
\foreach \mx/\my in {5.45/2.32, 5.95/1.96, 6.45/1.78} {
  \node[circle, draw=cRed!85, fill=cRed!25, inner sep=0pt,
        minimum size=0.9mm, line width=0.25pt] at (\mx,\my) {};
}
