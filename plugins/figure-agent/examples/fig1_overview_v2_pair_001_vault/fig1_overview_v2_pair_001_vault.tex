...
\draw[cGray!75!black, line width=0.28pt] (3.50, 3.20) -- (3.65, 3.20) -- (3.65, 3.06);
\draw[cGray!75!black, line width=0.30pt] (3.57, 3.06) -- (3.73, 3.06);
\draw[cGray!75!black, line width=0.25pt] (3.60, 3.03) -- (3.70, 3.03);
\draw[cGray!75!black, line width=0.22pt] (3.62, 3.00) -- (3.68, 3.00);
...
% iter 52d: Debye label moved INSIDE plot (just above x-axis, right of dashed
% curve endpoint) so it no longer reads as x-tick AND leader doesn't cross
% axis (Gemini iter 52c HIGH: axis-crossing leader looked like "curve crashing
% through axis"). At y=0.45 anchor=west the label fits in narrow strip above
% axis (y=0.40) without colliding with shallow-rich curve (y at x=2.60..3.10
% is 1.04..0.82, label top at ~0.70 stays clear). Color-matched gray to
% dashed curve, no leader needed (adjacency + color = unambiguous identifier).
