...
| TG-G-001 | BLOCKER | Panel G **result zone** is Coulomb-only; Maxwell attraction and actuator framing are absent from result zone. | Source/render text and arrow review of Column F result zone (post-v8.6) / Panel G (pre-v8.6). | (v8.6 update): scope narrowed to *result zone* per §8.5 amendment; apparatus zone now LEGITIMATELY shows Maxwell baseline per TG-G-002. Pass for this loop: result zone remains Coulomb-only; no actuator framing. |
| TG-G-002 | BLOCKER | Column F (v8.6 mechanical column) Maxwell-vs-Coulomb contrast convention: apparatus zone shows baseline Maxwell attraction on neutral polymer (light pink dashed, cRed!35!white, 0.35pt); result zone shows Coulomb repulsion winning against this baseline (bold cRed!80 solid, 0.7pt). Color/weight tier asymmetry MUST signal "Coulomb wins." | Source/render review of Column F apparatus + result zone color, weight, dashed/solid discrimination. | (v8.6 NEW): pending v8.6 .tex implementation. After implementation, verify (a) Maxwell arrow exists in apparatus zone with correct light styling, (b) Coulomb arrow exists in result zone with correct bold styling, (c) Maxwell arrow does NOT appear in result zone, (d) color tier reads "Maxwell < Coulomb" at peripheral vision. |
