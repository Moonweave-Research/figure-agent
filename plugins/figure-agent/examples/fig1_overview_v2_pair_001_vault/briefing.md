...
- **E-2** apparatus zone — ISPD-apparatus scene at column-top, **He et al Nat Commun 2024 Fig 1c adaptation** per v8.7 (direct reference match). Layout: corona needle (LEFT, charging step) + sample slab on grounded base (CENTER) + Kelvin probe with support (RIGHT, V_s readout) + small instrument box (FAR-RIGHT, "V_s meter"). Mono cGray!75!black 0.30pt stroke. Width ~3.7cm × height ~1.30cm.
  ...
  - **Sample stack (2-layer, v8.7 Column-E iter2 2026-05-18 upgrade)** at CENTER: polymer film (cAmber!28 fill + cAmber!70!black 0.30pt outline) at top y=3.45..3.52 (0.07cm thin film, x=6.30..7.85) + **substrate** (cGray!18 fill + cGray!75 0.30pt outline + diagonal hatching cGray!55 0.14pt for material identification) at bottom y=3.34..3.45 (0.11cm). Substrate represents the conductive base required for ISPD charge-decay path. Ground attaches to substrate (not polymer) at right edge y=3.39 — physically accurate.
  ...
  - **Kelvin probe (v8.7 Column-E iter6 disk-on-shaft redesign per Tech illustrator audit)**: vertical shaft (cGray!75 0.32pt) from y=3.95 to 3.66 + disk electrode at bottom (flat horizontal cGray!50!white fill, 0.30×0.04cm at y=3.62..3.66, slightly wider than shaft) + **vertical ↕ vibration arrow ON shaft** (cGray!65 0.32pt, y=3.76..3.88) — Kelvin-probe modulation signature is vertical oscillation along shaft axis (FM-CPD measurement defining feature). Prior iter2 design (horizontal block + arcs above) was Tech-illustrator-flagged as "hot plate + RF wireless"; iter6 replaces with proper disk-on-shaft. **Explicit air gap indicator** (dashed vertical line + `d` label) between disk bottom (y=3.62) and polymer top (y=3.52) — non-contact measurement geometry (~100-500 μm in reality). Label `Probe` italic 5pt at (7.62, 3.82).
