...
- **E-2** apparatus zone — ISPD-apparatus scene at column-top, **He et al Nat Commun 2024 Fig 1c adaptation** per v8.7 (direct reference match). Layout: corona needle (LEFT, charging step) + sample slab on grounded base (CENTER) + Kelvin probe with support (RIGHT, V_s readout) + small instrument box (FAR-RIGHT, "V_s meter"). Mono cGray!75!black 0.30pt stroke. Width ~3.7cm × height ~1.30cm.
  ...
  - **Kelvin probe (v8.7 Column-E iter2 upgrade)** at RIGHT `(7.20, 3.60)`..`(7.60, 3.73)`: enlarged probe head (0.40×0.13cm, was 0.30×0.10) cGray!35 fill + cGray!75 0.32pt outline + thin support stem to (7.40, 3.85) + **2 vibration arcs above probe** (cGray!65 0.25/0.20pt curves at y=3.78..3.82) — Kelvin-probe modulation signature (frequency-modulated CPD measurement, the defining instrument feature). Label `Probe` italic 5pt.
  - **V_s meter box (v8.7 Column-E iter2 upgrade)** at FAR-RIGHT `(8.20, 3.65)`..`(9.00, 4.05)`: rectangle with **small sinusoidal waveform inside top** (cGray!60 0.22pt, ~3 cycles at y≈3.93) — measurement-display cue indicating time-varying signal readout. `$V_s$ meter` italic 5pt label in lower portion. Lead from probe support to meter LEFT side (cGray!75 0.22pt) at y=3.85.
