...
- **B-4** sample boundary divider lines (v8.4 NEW, v8.7 iter1 updated): 3 chains 사이에 2개의 thin horizontal `cGray!40, line width=0.18pt, densely dotted` divider lines (post-v8.7 critique C002 fix: tone 25→40), x extent 동일 (3.85..6.30), y at midpoints between adjacent chains (7.45 / 6.55). **Role**: 3개 chain row이 *3개 distinct representative sample* (S60/S75/S85) 임을 시각적으로 명시 — chain bundle 오독 risk 차단. divider는 *separation cue* only (NOT data axis), 따라서 dotted + 얕은 cGray로 inter-sample boundary만 표시. **Forbidden**: solid lines (axis tone 침범), color (sample-identity confusion), width > 0.20pt (mechanism-tier 침범)
...
- **E-2** apparatus zone — ISPD-apparatus scene at column-top, **He et al Nat Commun 2024 Fig 1c adaptation** per v8.7 (direct reference match). Layout: corona needle (LEFT, charging step) + sample slab on grounded base (CENTER) + Kelvin probe with support (RIGHT, V_s readout) + small instrument box (FAR-RIGHT, "V_s meter"). Mono cGray!75!black 0.30pt stroke. Width ~3.7cm × height ~1.30cm.
  ...
  - **Sample slab** at CENTER `(6.45, 3.40)`..`(7.85, 3.50)`: cAmber!28 fill + cGray!75 0.30pt outline + small ground stub on right end (3-bar ground at right edge).
  - **Kelvin probe** at RIGHT `(7.25, 3.65)`..`(7.55, 3.72)`: small rectangle hovering above sample, with thin support stem to box above. Label `Probe` italic 5.5pt.
  - **V_s meter box** at FAR-RIGHT `(8.10, 3.35)`..`(8.85, 3.85)`: rectangle labeled `$V_s$ meter` (italic 5.5pt) + small readout indicator. Lead from probe support to meter.
  - **Motion arrow** small `↔` dashed line below probe indicating scan path (optional, if space permits).
  - **Role**: apparatus zone hero — shared by raw V_s + derived g(E_t) per §6 paired ISPD. Direct He 2024 Fig 1c idiom transfer. Replaces v8.6 simple `corona + Kelvin probe + slab` mini-scene.
