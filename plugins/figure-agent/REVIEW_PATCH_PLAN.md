...
- [ ] **D2** `scripts/critique_brief.py`가 `spec.yaml.reference_image`를 무시.
...
- [ ] **D5** TRACKED_GOLDEN ↔ `/fig_status stale_export` 안내 충돌.
...
- [ ] **D6** `coordinate_hints.yaml`이 `extraction_version: 0.1`로 남아도 stale 미감지.
