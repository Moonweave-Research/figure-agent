...
The v20 physics sanity inventory starts a separate basic-academic correctness layer. It audits energy ordering, trap ordering, decay notation, Debye-to-`g(Et)` causal semantics, reference authority, claim overreach, and probe charge/electrode/force-vector consistency. This is not publication-grade theory validation; it is a guard against clearly wrong sign, direction, ordering, model-chain, and claim errors.
