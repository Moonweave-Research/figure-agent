...
The v24 render parity gate adds `src/verify_fig1_render_parity.py` as a hard source/artifact synchronization check. It regenerates SVG text from the current `build_scene()` and `svg_text_for_scene()` path, then compares it byte-for-byte against the tracked `fig1_reference_semantic.svg`. This catches renderer, scaffold, or payload source drift that would otherwise pass a baseline hash check against an old checked-in SVG. It does not compare PNG files, does not add subjective visual rules, and does not replace `src/verify_fig1_baseline_hash.py`; render parity and baseline hash are orthogonal.

The v25 origin payload visibility pass is a narrow semantic-to-visual polish pass. The sulfur polymer origin panel now renders the typed payload labels `Heat 160 C` and `-Sx- chain` instead of renderer-local literals, and `src/test_fig1_origin_payload_visibility.py` protects that payload visibility path. It does not change the scaffold, does not add new semantic content, does not add strict aesthetic rules, and still requires human visual review before publication-grade approval.
...
python experiments/python_svg_semantic_fig1/src/verify_fig1_render_parity.py
