...
class Fig1PhysicsSanityTests(unittest.TestCase):
    def assert_physics_failure(
        self, scene: object, token: str, *, extra_claim_text: str = ""
    ) -> None:
    ...
    def test_current_scene_locks_cantilever_target_and_passes_probe_sanity(
        self,
    ) -> None:
        scene = build_scene()
    ...
    def test_rejects_ground_truth_reference_authority(self) -> None:
        scene = build_scene()
        bad_scene = replace(
            scene, reference=replace(scene.reference, authority="ground_truth")
        )

        self.assert_physics_failure(bad_scene, "ground_truth")

    def test_accepts_non_ground_truth_reference_authority_without_taxonomy_coupling(
        self,
    ) -> None:
        scene = build_scene()
        future_scene = replace(
            scene, reference=replace(scene.reference, authority="composition_evidence")
        )

        self.assertFalse(
            any(
                "reference authority" in failure
                for failure in physics_sanity_failures(future_scene)
            ),
    ...
    def test_rejects_lumo_homo_label_or_position_inversion(self) -> None:
        ...
        self.assert_physics_failure(
            scene.replace_payload("band_diagram", bad_label_band), "LUMO/HOMO"
        )
        self.assert_physics_failure(
            scene.replace_payload("band_diagram", bad_position_band), "LUMO/HOMO"
        )
    ...
    def test_rejects_trap_positions_outside_bandgap(self) -> None:
        ...
        bad_traps = replace(
            traps, shallow_positions=(0.05, *traps.shallow_positions[1:])
        )

        self.assert_physics_failure(
            scene.replace_payload("trap_level_set", bad_traps), "bandgap"
        )
    ...
    def test_rejects_invalid_dos_numeric_parameters(self) -> None:
        ...
        self.assert_physics_failure(
            scene.replace_payload("dos_lobes", bad_lobes), "DOS"
        )
    ...
    def test_rejects_invalid_ispd_numeric_parameters(self) -> None:
        ...
        self.assert_physics_failure(
            scene.replace_payload("ispd_plot", bad_ispd), "ISPD"
        )
    ...
    def test_rejects_invalid_power_law_decay_semantics(self) -> None:
        ...
        self.assert_physics_failure(
            scene.replace_payload("power_law_decay", bad_slope), "power-law"
        )
        self.assert_physics_failure(
            scene.replace_payload("power_law_decay", bad_label), "t^-n"
        )
        self.assert_physics_failure(
            scene.replace_payload("power_law_decay", bad_parameter),
            "extracted parameter",
        )
    ...
    def test_rejects_invalid_causal_chain_claim(self) -> None:
        ...
        bad_flow = replace(
            flow,
            causal_chain=("I(t) ~ t^-n", "Debye exp(-t/tau)", "n", "tau_d", "g(Et)"),
        )

        self.assert_physics_failure(
            scene.replace_payload("trap_model_flow", bad_flow), "causal chain"
        )
    ...
    def test_rejects_like_charge_sign_condition_mismatch(self) -> None:
        ...
        self.assert_physics_failure(
            scene.replace_payload("electrode", bad_electrode), "like-charge"
        )

    def test_force_target_cantilever_rejects_rightward_vector_toward_positive_electrode(
        self,
    ) -> None:
        scene = build_scene()
        force = scene.object_by_id("repulsion_arrow").payload
        right_start, right_end = sorted(
            (force.start, force.end), key=lambda point: point.x
        )
    ...
    def test_force_target_electrode_allows_current_rightward_reaction_vector(
        self,
    ) -> None:
        scene = build_scene()
        force = scene.object_by_id("repulsion_arrow").payload
    ...
    def test_rejects_charge_positions_overlapping_electrode(self) -> None:
        ...
        self.assert_physics_failure(
            scene.replace_payload("polymer_cantilever", bad_cantilever),
    ...
    def test_rejects_force_arrow_start_far_from_charge_cluster(self) -> None:
        ...
        self.assert_physics_failure(
            scene.replace_payload("repulsion_arrow", far_force), "force arrow start"
        )
    ...
    def test_rejects_probe_bend_state_drift(self) -> None:
        ...
        self.assert_physics_failure(
            scene.replace_payload("polymer_cantilever", bad_cantilever), "bend"
        )
    ...
    def test_rejects_maxwell_cue_role_or_direction_drift(self) -> None:
        ...
        self.assert_physics_failure(
            scene.replace_payload("maxwell_attraction_cue", bad_role), "Maxwell"
        )
        self.assert_physics_failure(
            scene.replace_payload("maxwell_attraction_cue", bad_direction), "Maxwell"
        )
