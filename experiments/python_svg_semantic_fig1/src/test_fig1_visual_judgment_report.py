...
class Fig1VisualJudgmentReportTests(unittest.TestCase):
    ...
    def test_report_documents_v21_cantilever_reference_divergence(self) -> None:
        ...
        divergence = next(
            ...
            if category["name"] == "Reference Divergence"
        )
    ...
    def test_reference_divergence_reflects_force_target_mutation(self) -> None:
        ...
        drifted_scene = scene.replace_payload(
            "repulsion_arrow", replace(force, force_target="electrode")
        )
        ...
        self.assertIn(
            "v21 contract expects force_target=cantilever and leftward=True", joined
        )
        self.assertNotIn("intentional v21 divergence retained", joined)
        self.assertTrue(
            any(item["severity"] == "possible issue" for item in divergence["items"])
        )
    ...
    def test_expected_composite_labels_are_not_reported_as_text_collisions(
        self,
    ) -> None:
        report = build_visual_judgment_report(build_scene(), SVG)
        text_collision = next(
            category
            for category in report["categories"]
            if category["name"] == "Text / Text Near-Collision"
        )
    ...
    def test_human_review_prompts_include_multiple_risk_categories(self) -> None:
        ...
        prompts = next(
            category
            for category in report["categories"]
            if category["name"] == "Human Review Prompts"
        )
    ...
    def test_visual_hierarchy_bbox_salience_proxy_is_evidence_only(self) -> None:
        ...
        hierarchy = next(
            category
            for category in report["categories"]
            if category["name"] == "Visual Hierarchy"
        )
        salience_items = [
            item
            for item in hierarchy["items"]
            if "bbox salience proxy" in item["message"]
        ]
    ...
    def test_json_sidecar_is_not_required_by_docs_manifest(self) -> None:
        from check_fig1_docs_manifest import (
        ...
        self.assertNotIn(
            "fig1_visual_judgment_report.json", REQUIRED_V22_VISUAL_REPORT_TOKENS
        )
    ...
    def test_report_is_not_added_to_strict_gate_runner(self) -> None:
        ...
        self.assertNotIn(
            "report_fig1_visual_judgment.py", {gate.script for gate in GATES}
        )

    def test_records_include_local_box_id_for_subregion_attribution(self) -> None:
        report = build_visual_judgment_report(build_scene(), SVG)

        for record in report["text_boxes"]:
            self.assertIn("local_box_id", record)
        for record in report["semantic_boxes"]:
            self.assertIn("local_box_id", record)

        localized = [
            record
            for record in (*report["text_boxes"], *report["semantic_boxes"])
            if record["panel_id"] == "localized_traps"
        ]
        sub_attributed = [record for record in localized if record["local_box_id"]]
        self.assertGreater(
            len(sub_attributed),
            0,
            "expected at least one localized_traps record to bind to band_area/dos_area/hero_callout",
        )

    def test_text_shape_conflict_findings_mention_sub_region_when_available(
        self,
    ) -> None:
        report = build_visual_judgment_report(build_scene(), SVG)
        conflict = next(
            category
            for category in report["categories"]
            if category["name"] == "Text / Shape Conflict"
        )
        joined = " ".join(item["message"] for item in conflict["items"])

        self.assertTrue(
            any(token in joined for token in ("band_area", "dos_area", "hero_callout")),
            f"expected sub-region attribution in text/shape conflict messages: {joined}",
        )

    def test_text_near_collision_findings_mention_sub_region_when_available(
        self,
    ) -> None:
        report = build_visual_judgment_report(build_scene(), SVG)
        collision = next(
        ...
        joined = " ".join(item["message"] for item in collision["items"])

        self.assertTrue(
            any(token in joined for token in ("band_area", "dos_area", "hero_callout")),
            f"expected sub-region attribution in text/text near-collision messages: {joined}",
        )
